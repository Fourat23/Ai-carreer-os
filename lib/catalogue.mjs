...
// Parcours annoncés (pas encore implémentés — non activables). Le parcours
// « cloud-devops-v1 » a été promu en parcours DISPONIBLE, piloté par données
// (« cloud-devops-engineer-v1 », V26) ; il quitte donc la liste des annoncés.
...
// Parcours Data/ML Engineer (V35). SÉLECTIONNE, par COMPÉTENCE, les journées de la
// colonne vertébrale data/IA du programme (terminal/git, Python & données, SQL,
// ML, deep learning, transformers/LLM, IA appliquée & évaluation) — sélection NON
// CONTIGUË (mécanisme existant), aucun jour ni contenu dupliqué. Exclut
// volontairement le frontend/JS, le web/backend non essentiel et les soft-skills :
// identité distincte du parcours AI Engineer Foundations (365 j). Durée dérivée.
export const DATA_ML_TRACK_ID = 'data-ml-v1';
const DATA_ML_TECHS = ['python', 'sql', 'architecture', 'llm', 'evaluation'];
// [id, titre, résumé, compétences sélectionnées]
const DATA_ML_MODULE_SPECS = [
  ['dml-01-foundations', 'Fondations : terminal & Git', 'Environnement de travail, shell et Git pour manipuler données et scripts de façon reproductible.', ['gitlinux']],
  ['dml-02-python-data', 'Python & manipulation de données', 'Python, structures tabulaires (pandas), nettoyage/qualité et pipelines ETL : préparer une donnée exploitable.', ['python']],
  ['dml-03-sql', 'SQL & modèle relationnel', 'Interroger, modéliser et agréger des données relationnelles.', ['sql']],
  ['dml-04-ml', 'Statistiques & machine learning', 'Statistiques intuitives, features/target, split, feature engineering, ML supervisé/non supervisé et évaluation.', ['ml']],
  ['dml-05-deep-learning', 'Deep learning', 'Réseaux de neurones : neurone, loss, gradient, entraînement et diagnostic.', ['dl']],
  ['dml-06-transformers-llm', 'Transformers & LLM', 'Embeddings, attention, transformers puis grands modèles de langage.', ['llm']],
  ['dml-07-applied-ai', 'IA appliquée, évaluation & production', 'RAG, agents, évaluation des systèmes IA et mise en production (coûts, observabilité, dérive).', ['rag', 'agents', 'evalia']],
];
...
// Parcours Frontend Engineer (V36). SÉLECTIONNE un spine frontend FOCALISÉ du
// trimestre 1 : langage (JS 8-14, TS 36-49) puis tout le bloc React/tests/projet
// (87-119). NON CONTIGU : exclut volontairement l'algorithmie (15-35) et tout le
// backend/HTTP/SQL/sécurité (50-86) — identité distincte du Full-Stack (qui les
// inclut) et du parcours AI Engineer (365 j). Le socle « Web Platform » (HTML,
// CSS, layout, responsive) créé en V36 vit dans les LEÇONS canoniques (corpus,
// graphe, /lessons) reliées à ces jours ; aucun jour n'est créé ni dupliqué.
export const FRONTEND_TRACK_ID = 'frontend-engineer-v1';
// [id, titre, résumé, from, to, lessonRefs] — lessonRefs rattache les LEÇONS
// canoniques pertinentes au module (V37 reachability) : le socle « Web Platform »
// (HTML/CSS/Flexbox/Grid/responsive) n'a pas de jours dédiés dans le programme 365 j,
// on le rend donc atteignable DEPUIS le parcours via ce rattachement additif.
const FRONTEND_MODULE_SPECS = [
  ['fe-01-js', 'JavaScript moderne', 'Tableaux, fonctions, objets, asynchrone — les fondamentaux du langage de l\'interface.', 8, 14, ['javascript-basics', 'async-javascript']],
  ['fe-02-ts', 'TypeScript & conception', 'Typage, types avancés, POO, patterns, clean code, débogage — fiabiliser le code d\'interface.', 36, 49, ['typescript-basics', 'typescript-frontend']],
  ['fe-03-web-react-intro', 'Plateforme web & introduction à React', 'Le socle web visuel (HTML sémantique, CSS, Flexbox, Grid, responsive) et le DOM, puis l\'introduction à React et la consommation d\'API (fetch et états async).', 87, 91, ['html-semantic-structure', 'css-fundamentals', 'css-flexbox', 'css-grid', 'responsive-design', 'browser-dom-rendering', 'react-fundamentals']],
  ['fe-04-react-core', 'React : composants, état et hooks', 'Composants/props/JSX, state (useState), rendu conditionnel et listes, effets (useEffect), formulaires contrôlés, gestion d\'erreur centralisée.', 92, 98, ['react-hooks-effects', 'web-forms-validation']],
  ['fe-05-react-app', 'Application React : routing, état partagé, performance, accessibilité', 'Routing et navigation, remontée et partage d\'état, Context, performance (re-renders), accessibilité et UX.', 99, 105, ['react-composition-architecture', 'react-application-states', 'react-accessibility']],
  ['fe-06-tests', 'Tests & qualité front', 'Tests unitaires, tests de composants React, mocks et intégration, clean code appliqué au front, hooks personnalisés, gestion d\'erreur robuste.', 106, 112, ['frontend-testing', 'frontend-performance']],
  ['fe-07-project', 'Projet 3 — BiblioApp (intégrateur)', 'Construire une application front complète : socle, CRUD, recherche et filtres, tests, polish et états limites, README/ADR/démo. Ouverture Next.js.', 113, 119, ['nextjs-foundations']],
];
const FRONTEND_TECHS = ['javascript', 'typescript', 'react', 'testing'];

/** Modules du parcours Frontend : plages de jours réelles, skills dérivés, leçons rattachées. */
function frontendModules(program) {
  const days = program?.days ?? [];
  return FRONTEND_MODULE_SPECS.map(([id, title, summary, from, to, lessonRefs]) => {
    const mdays = days.filter((d) => d.day >= from && d.day <= to).sort((a, b) => a.day - b.day);
    return { id, title, summary, dayRefs: mdays.map((d) => d.day), skills: uniq(mdays.map((d) => d.skill).filter(Boolean)), lessonRefs: lessonRefs ?? [], projectRef: null };
  });
}
...
// [id, titre, résumé, from, to, lessonRefs] — lessonRefs (V38) rend les leçons
// canoniques backend/system-design suivables DEPUIS le parcours (additif ; les
// contenus API-production/async/system-design/distribués n'ont pas de jours dédiés).
const BACKEND_MODULE_SPECS = [
  ['be-01-foundations', 'Fondations backend (terminal, Git, JS)', 'Terminal, Git et JavaScript pour le développement serveur.', 1, 14, ['terminal-shell-filesystem', 'git-fundamentals', 'javascript-basics']],
  ['be-02-algo', 'Algorithmie & structures de données', 'Complexité, recherche, récursion, Map/Set, piles/files.', 15, 35, ['algorithmic-thinking', 'data-structures-intro']],
  ['be-03-typescript', 'TypeScript & conception', 'Typage, POO, patterns, clean code, débogage — Projet 1.', 36, 49, ['typescript-basics', 'clean-code', 'design-patterns-intro']],
  ['be-04-http-node', 'HTTP, Node & Express', 'Le protocole HTTP, la conception REST, Node natif, Express, validation et erreurs.', 50, 56, ['http-rest-json', 'api-design-basics', 'express-backend', 'api-production-contracts']],
  ['be-05-sql-project', 'SQL, persistance & API (Projet 2)', 'SQL, modélisation, transactions — une API réellement persistée.', 57, 66, ['sql-foundations', 'database-modeling', 'database-transactions-concurrency', 'sql-performance-indexing', 'database-migrations']],
  ['be-06-security', 'Sécurité applicative', 'OWASP appliqué, authentification par token, gestion des secrets.', 67, 70, ['authentication', 'breaking-changes-compatibility']],
  ['be-07-architecture', 'Architecture, observabilité & qualité', 'Réseau, terminal/Git avancés, architecture 3-tiers/MVC, observabilité, cache, trade-offs.', 71, 81, ['architecture-basics', 'caching-performance', 'async-messaging-queues', 'system-design-scaling', 'distributed-systems-failures', 'resilience-patterns']],
  ['be-08-consolidation', 'Consolidation & durcissement', 'Consolidation, durcissement et performance de l’API (Projet 2).', 83, 86, ['technical-debt', 'refactoring-legacy-code', 'technical-documentation']],
];
const BACKEND_TECHS = ['linux', 'git', 'javascript', 'typescript', 'node', 'api', 'sql', 'testing', 'architecture'];

/** Modules du parcours Backend : plages de jours réelles, skills dérivés, leçons rattachées. */
function backendModules(program) {
  const days = program?.days ?? [];
  return BACKEND_MODULE_SPECS.map(([id, title, summary, from, to, lessonRefs]) => {
    const mdays = days.filter((d) => d.day >= from && d.day <= to).sort((a, b) => a.day - b.day);
    return { id, title, summary, dayRefs: mdays.map((d) => d.day), skills: uniq(mdays.map((d) => d.skill).filter(Boolean)), lessonRefs: lessonRefs ?? [], projectRef: null };
  });
}
...
// Parcours Cloud / DevOps Engineer (V26). RÉUTILISE des journées existantes qui
// forment le cycle build → ship → run : fondations système/shell/Git, réseau &
// services, sécurité & secrets, architecture/cloud/haute disponibilité,
// durcissement & fiabilité, conteneurs & Kubernetes, CI/CD & livraison. AUCUN jour
// créé, AUCUN contenu copié ; durée DÉRIVÉE des jours résolus. La profondeur
// canonique de ce parcours est portée par les Leçons de fond Cloud/DevOps ajoutées
// en V26 (Linux, réseau, Docker, CI/CD, Kubernetes, cloud/AWS/Azure/IaC/FinOps),
// reliées aux journées via les compétences. Cadré junior/entrée : la dette de
// journées de PRATIQUE dédiées (K8s/AWS/Azure/IaC/FinOps) reste documentée pour
// V27/V28 (cf. docs/PEDAGOGICAL-AUDIT-V26.md).
export const CLOUD_DEVOPS_TRACK_ID = 'cloud-devops-engineer-v1';
const CLOUD_DEVOPS_MODULE_SPECS = [
  ['cde-01-foundations', 'Fondations système, shell & Git', 'Environnement, terminal, arborescence, redirections/pipes et Git : le socle opérationnel.', 1, 7],
  ['cde-02-network-services', 'Réseau, HTTP & services', 'Pile réseau (DNS/TCP/TLS/HTTP) et exposition de services Node/Express : ce qu\'on déploie et diagnostique.', [50, 51, 52, 53, 54, 55, 56, 71]],
  ['cde-03-security-secrets', 'Sécurité & secrets', 'OWASP appliqué, authentification par token, gestion des secrets et moindre privilège.', 67, 68],
  ['cde-04-architecture-cloud', 'Architecture, cloud & haute disponibilité', 'Architecture 3-tiers déployée dans le cloud (IaaS/PaaS, responsabilité partagée, zones), haute disponibilité (SPOF, failover, SLO), scalabilité et stratégies de déploiement.', 78, 81],
  ['cde-05-hardening', 'Durcissement & fiabilité', 'Consolidation, durcissement et performance : rendre un service fiable, exploitable et observable.', 83, 86],
  ['cde-06-containers-k8s', 'Conteneurs & Kubernetes', 'Dockerisation reproductible puis orchestration Kubernetes : état désiré/observé, objets, probes, rollout/rollback et incidents (CrashLoopBackOff/OOMKilled).', 320, 321],
  ['cde-07-cicd', 'CI/CD & livraison continue', 'Ingénierie de livraison : anatomie d\'un pipeline, artefacts/cache, secrets/environnements, promotion et rollback — raisonner une chaîne de livraison déterministe.', [307, 326]],
];
const CLOUD_DEVOPS_TECHS = ['git', 'linux', 'api', 'architecture', 'docker', 'cloud', 'testing', 'security'];

/** Modules du parcours Cloud/DevOps : mêmes règles génériques (plages ou jours explicites, skills dérivés). */
function cloudDevopsModules(program) {
  const days = program?.days ?? [];
  return CLOUD_DEVOPS_MODULE_SPECS.map(([id, title, summary, from, to]) => {
    const inModule = Array.isArray(from)
      ? (d) => from.includes(d.day)
      : (d) => d.day >= from && d.day <= to;
    const mdays = days.filter(inModule).sort((a, b) => a.day - b.day);
...
/**
 * Modules du parcours Data/ML : sélection par COMPÉTENCE (non contiguë), dérivée
 * des journées réelles. Chaque module regroupe les jours dont le skill figure dans
 * la liste du module. Aucun jour dupliqué (skills disjoints par jour), aucune copie.
 */
function dataMlModules(program) {
  const days = program?.days ?? [];
  return DATA_ML_MODULE_SPECS.map(([id, title, summary, skills]) => {
    const wanted = new Set(skills);
    const mdays = days.filter((d) => wanted.has(d.skill)).sort((a, b) => a.day - b.day);
    return { id, title, summary, dayRefs: mdays.map((d) => d.day), skills: uniq(mdays.map((d) => d.skill).filter(Boolean)), projectRef: null };
  });
}
...
export function buildCatalogue(program) {
  ...
  const cdeList = cloudDevopsModules(program);
  const dmlList = dataMlModules(program);
  const feList = frontendModules(program);
  const modules = {};
  for (const m of modList) modules[m.id] = m;
  for (const m of fstList) modules[m.id] = m;
  for (const m of feList) modules[m.id] = m;
  for (const m of beList) modules[m.id] = m;
  for (const m of scList) modules[m.id] = m;
  for (const m of acsList) modules[m.id] = m;
  for (const m of dmlList) modules[m.id] = m;
  for (const m of cdeList) modules[m.id] = m;
  ...
  const cloudDevopsDayCount = new Set(cdeList.flatMap((m) => m.dayRefs)).size;
  const cloudDevops = {
    id: CLOUD_DEVOPS_TRACK_ID,
    version: '1',
    status: 'available',
    title: 'Cloud / DevOps Engineer',
    goal: 'Maîtriser le cycle build → ship → run : fondations système et réseau, sécurité et secrets, architecture cloud et haute disponibilité, durcissement, conteneurs et Kubernetes, CI/CD et livraison continue — jusqu\'à déployer et exploiter un service de façon fiable et raisonnée.',
    roles: ['Ingénieur DevOps junior', 'Ingénieur cloud junior', 'Futur profil SRE / plateforme junior'],
    moduleRefs: cdeList.map((m) => m.id),
    technologies: CLOUD_DEVOPS_TECHS,
    totalDays: cloudDevopsDayCount,
    completion: { minDaysDone: cloudDevopsDayCount },
  };

  const dataMlDayCount = new Set(dmlList.flatMap((m) => m.dayRefs)).size;
  const dataMl = {
    id: DATA_ML_TRACK_ID,
    version: '1',
    status: 'available',
    title: 'Data / ML Engineer — Foundations',
    goal: 'Aller de « qu\'est-ce qu\'une donnée » à un système IA en production : Python & manipulation de données, SQL, statistiques et machine learning, évaluation, deep learning, transformers/LLM, puis IA appliquée (RAG, agents) et sa mise en production.',
    roles: ['Data Analyst junior orienté ML', 'Data/ML Engineer junior', 'AI Engineer junior orienté données'],
    moduleRefs: dmlList.map((m) => m.id),
    technologies: DATA_ML_TECHS,
    totalDays: dataMlDayCount,
    completion: { minDaysDone: dataMlDayCount },
  };

  const frontendDayCount = new Set(feList.flatMap((m) => m.dayRefs)).size;
  const frontend = {
    id: FRONTEND_TRACK_ID,
    version: '1',
    status: 'available',
    title: 'Frontend Engineer',
    goal: 'Construire des interfaces web modernes, accessibles et robustes : plateforme web (HTML sémantique, CSS, mise en page, responsive), JavaScript/TypeScript côté navigateur, React (composants, état, hooks, application), tests et performance frontend.',
    roles: ['Développeur Frontend React/TypeScript junior', 'Intégrateur web junior', 'Développeur d\'interfaces junior'],
    moduleRefs: feList.map((m) => m.id),
    technologies: FRONTEND_TECHS,
    totalDays: frontendDayCount,
    completion: { minDaysDone: frontendDayCount },
  };
  ...
  const catalogue = { technologies: TECHNOLOGIES, modules, tracks: [foundations, fullstack, frontend, backend, systemsCloud, appsecCloud, cloudDevops, dataMl, ...announced] };
...
/**
 * Liste CHRONOLOGIQUE et dédupliquée des jours d'un parcours. PURE.
 * La durée d'un parcours en découle.
 *
 * V54.2.1 — l'implémentation concaténait les `dayRefs` module par module et
 * rendait donc l'ordre des MODULES, pas l'ordre du programme. Les modules
 * d'un parcours ne sont pas nécessairement déclarés dans l'ordre des journées
 * (ni disjoints), si bien que des parcours sous-ensembles (Data/ML, Systems &
 * Cloud, AppSec, Cloud/DevOps) produisaient des listes comme `… 73, 82, 84,
 * 57, 58 …`. Tout ce qui en dérive — calendrier, trajectoire, voisins de
 * navigation, position — héritait de ce désordre.
 *
 * L'ordre pédagogique d'une journée est son rang dans le Curriculum 1.0 (les
 * 365 jours gelés). Un module est un REGROUPEMENT, pas une réorganisation :
 * le tri croissant est donc la seule lecture correcte, et il ne modifie ni le
 * contenu, ni l'appartenance, ni la durée d'un parcours.
...
export function resolveTrackDays(catalogue, trackOrId) {
  ...
  for (const ref of track.moduleRefs ?? []) {
    ...
    if (!m) continue;
    for (const d of m.dayRefs ?? []) seen.add(d);
  }
