// CP3+ (V33) — contrat des exercices de mécanique d'agent, vérifié par EXÉCUTION :
...
const V33_EXERCISES = [
  ...
  'attention-argmax', 'llm-cost-estimate', 'ml-confusion-metric',
];
...
test('V33 : les exercices d\'agent existent et sont des fixtures valides', () => {
  for (const id of V33_EXERCISES) {
...
test('V33 : compétences connues, ≥1 test public et ≥1 test privé, référence présente, SIMULATION', () => {
  for (const id of V33_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V33 : référence 100% verte, starter échoue ≥1 test public (exécuté)', async () => {
  for (const id of V33_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
