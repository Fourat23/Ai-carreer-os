...
// Seule dépendance, et elle est PURE : la forme d'une provenance. La recopier
// ici donnerait deux définitions du même champ — le défaut que V77 · CP3 a payé
// sur l'énumération des faits.
import { normalizeProvenance } from './event-model.mjs';
...
export function normalizeAttempt(raw) {
  ...
  return {
    ...
    // ── V77.1 · CP4 — LA PROVENANCE, ENFIN ÉCRITE ──
    //
    // `lib/event-model.mjs` déclare depuis V74 que `RecallAttempt` porte une
    // provenance. La répétition à blanc du CP4 a mesuré qu'elle ne l'écrivait
    // PAS : le normaliseur ne gardait pas le champ, et une tentative de rappel
    // exportée ne disait donc pas QUI l'avait constatée. Le modèle promettait,
    // le fait se taisait.
    //
    // Mode LEGACY, et c'est le point délicat : une tentative ancienne sans
    // provenance devient `producer: 'legacy'` — visible plutôt que muette — et
    // surtout elle n'est PAS rejetée. `normalizeAttempts` jette ce que ce
    // normaliseur refuse ; exiger la provenance aurait effacé l'historique de
    // rappel de tout le monde, en silence. C'est `H13`, et il serait passé
    // inaperçu.
    provenance: normalizeProvenance(raw.provenance, { legacy: true }),
  };
}
...
/**
 * Décompte par état — sur les seules notions RENCONTRÉES.
 *
 * Défaut trouvé au CP14, en lisant la page produite plutôt que le code : avec
 * une progression vierge, `/retention` affichait « Nouveau 127 · Fragile 1 »
 * ET, deux lignes plus bas, « 128 notions ne sont pas encore dans le décompte ».
 * Les mêmes notions étaient comptées des deux côtés. La page se contredisait
 * elle-même, et sa propre légende disait « Nouveau — RENCONTRÉ, jamais mis à
 * l'épreuve » : le nombre affirmait le contraire du texte qui l'expliquait.
 * C'est exactement la faute que V65.1 a passé un sprint à supprimer.
 *
 * Une notion entre dans le décompte si le produit l'a présentée (journée
 * ouverte) OU si l'apprenant l'a déjà tentée. Tout le reste est « pas encore
 * rencontré », et c'est une grandeur SÉPARÉE, jamais un état d'apprentissage.
 */
export function retentionCounts(projection) {
  const counts = { nouveau: 0, fragile: 0, en_consolidation: 0, retenu: 0, a_revoir: 0 };
  for (const p of projection) {
    if (!isEncountered(p)) continue;
...
export function isEncountered(p) {
  return Boolean(p?.exposure?.exposed) || (p?.recall?.attemptCount ?? 0) > 0;
}
