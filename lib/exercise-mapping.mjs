 ...
 * @param {string[]} [ctx.declarantsHorsCorpus]  V77 · CP8 — déclaration d'auteur
 *        vivant HORS du corpus gelé (`data/exercise-declarations.json`, règle R1b)
...
export function resoudreExercice({
  ...
  declarantsHorsCorpus = [],
} = {}) {
  const decl = [...new Set(liste(declarants))];

  // ── V77 · CP8 · R1b — DÉCLARATION D'AUTEUR **HORS CORPUS GELÉ** ──
  //
  // Le CP0 a mesuré la cause unique des 125 ambigus : un fichier d'exercice ne
  // porte ni `conceptIds` ni `lessonRefs`, et le corpus est GELÉ. Un auteur ne
  // pouvait donc pas trancher sans modifier le curriculum — ce que le brief
  // interdit. `data/exercise-declarations.json` lui ouvre cette porte.
  //
  // La règle vient APRÈS R1 : une déclaration déjà présente dans le corpus fait
  // foi, et un fichier annexe ne doit pas pouvoir la contredire en silence.
  // Elle est NOMMÉE à part, et non fondue dans R1, pour qu'une résolution reste
  // auditable : savoir qu'un rattachement vient d'un fichier annexe change ce
  // qu'on peut en dire.
  //
  // **Le CP8 livre ce fichier VIDE**, et c'est le résultat honnête : les quatre
  // sources auditées n'en tranchent aucun. Le mécanisme existe pour un auteur,
  // pas pour combler une statistique.
  const hors = [...new Set(liste(declarantsHorsCorpus))];
  if (decl.length === 0 && hors.length === 1) {
    return { concepts: hors, classe: 'UNAMBIGUOUS', regle: 'R1b · déclaré hors corpus par une seule leçon' };
  }
  if (decl.length === 0 && hors.length > 1) {
    return { concepts: hors, classe: 'MULTI_CONCEPT_BY_DESIGN', regle: `R1b · déclaré hors corpus par ${hors.length} leçons` };
  }
