...
function deriveReadiness(dims) {
  ...
  // V46 : cohérence avec le gate v44 — « strong-junior » exige une preuve
  // diagnostic PLEINE (assessment/capstone), pas seulement des misconceptions
  // (partial). Une pratique de code forte sans diagnostic complet plafonne à
  // junior-ready. Évite qu'ajouter des misconceptions (partial) sur-classe une
  // compétence.
  if (allPartialPlus && full('transfer') && full('diagnostic') && executableAutonomy) return 'strong-junior';
  return 'junior-ready';
}
