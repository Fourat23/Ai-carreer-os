...
export const LESSONS = [
  ...
  { file: 'javascript-basics.md', title: 'JavaScript : les bases solides', cat: 'Fondations', level: 1, min: 50, skills: ['jsts'] },
  { file: 'algorithmic-thinking.md', title: 'La pensée algorithmique', cat: 'Fondations', level: 1, min: 50, skills: ['algo'] },
  ...
  // ── Web & backend ──
  { file: 'http-rest-json.md', title: 'HTTP, REST et JSON', cat: 'Web & backend', level: 1, min: 45, skills: ['http'] },
  ...
  { file: 'clean-code.md', title: 'Clean code', cat: 'Software engineering & architecture', level: 1, min: 45, skills: ['se'] },
  ...
  // ── Python & ML ──
  { file: 'python-foundations.md', title: 'Python : les fondations', cat: 'Python & ML', level: 1, min: 45, skills: ['python'] },
  ...
  { file: 'structured-outputs-tools.md', title: 'Sorties structurées et function calling', cat: 'IA appliquée', level: 2, min: 45, skills: ['llm', 'agents'] },
  { file: 'embeddings.md', title: 'Embeddings', cat: 'IA appliquée', level: 2, min: 40, skills: ['rag', 'dl'] },
  { file: 'rag-fundamentals.md', title: 'RAG : retrieval-augmented generation', cat: 'IA appliquée', level: 2, min: 55, skills: ['rag'] },
  { file: 'chunking-strategies.md', title: 'Stratégies de chunking', cat: 'IA appliquée', level: 2, min: 40, skills: ['rag'] },
  { file: 'vector-databases.md', title: 'Bases de données vectorielles', cat: 'IA appliquée', level: 2, min: 40, skills: ['rag'] },
  { file: 'retrieval-reranking.md', title: 'Retrieval, hybride et reranking', cat: 'IA appliquée', level: 3, min: 50, skills: ['rag'] },
  { file: 'ai-evaluation.md', title: 'Évaluer un système IA', cat: 'IA appliquée', level: 3, min: 55, skills: ['evalia'] },
  ...
  { file: 'linux-processes-signals.md', title: 'Linux : processus et signaux', cat: 'Systèmes & Linux', level: 2, min: 45, skills: ['gitlinux'], practiceRefs: [{ kind: 'exercise', id: 'sys-process-top-cpu' }, { kind: 'exercise', id: 'docker-exit-diagnosis' }] },
  { file: 'linux-services-systemd.md', title: 'Linux : services et systemd', cat: 'Systèmes & Linux', level: 2, min: 45, skills: ['gitlinux', 'cloud'] },
  { file: 'linux-resources-io.md', title: 'Linux : ressources, mémoire et I/O', cat: 'Systèmes & Linux', level: 3, min: 50, skills: ['gitlinux', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'sys-process-top-cpu' }, { kind: 'exercise', id: 'k8s-oom-risk' }] },
  ...
  { file: 'networking-dns.md', title: 'Réseau : DNS, la résolution de noms', cat: 'Réseau', level: 2, min: 45, skills: ['http', 'cloud'], practiceRefs: [{ kind: 'exercise', id: 'net-dns-resolve' }] },
  { file: 'networking-http-tls.md', title: 'Réseau : HTTP et TLS', cat: 'Réseau', level: 2, min: 50, skills: ['http', 'secu'], practiceRefs: [{ kind: 'exercise', id: 'net-http-status-class' }, { kind: 'exercise', id: 'http-status' }] },
  ...
  { file: 'docker-compose.md', title: 'Docker Compose : orchestrer une application multi-services', cat: 'Conteneurs & Docker', level: 2, min: 45, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'docker-port-mapping' }] },
  { file: 'docker-production-hardening.md', title: 'Docker : durcissement pour la production', cat: 'Conteneurs & Docker', level: 3, min: 50, skills: ['cloud', 'secu'], practiceRefs: [{ kind: 'exercise', id: 'docker-detect-root' }, { kind: 'exercise', id: 'docker-detect-secret' }, { kind: 'exercise', id: 'docker-exit-diagnosis' }] },
  // ── CI/CD & livraison ──
  { file: 'ci-cd-pipeline-anatomy.md', title: "CI/CD : anatomie d'un pipeline", cat: 'CI/CD & livraison', level: 2, min: 45, skills: ['cloud', 'se'], practiceRefs: [{ kind: 'exercise', id: 'cicd-critical-path' }, { kind: 'exercise', id: 'cicd-topo-order' }, { kind: 'exercise', id: 'cicd-fail-fast' }, { kind: 'exercise', id: 'cicd-cache-key' }] },
  ...
  { file: 'deployment-strategies.md', title: 'Stratégies de déploiement sans coupure', cat: 'CI/CD & livraison', level: 3, min: 50, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'cloud-deploy-strategy' }, { kind: 'exercise', id: 'k8s-rolling-available' }, { kind: 'exercise', id: 'cloud-readiness-routing' }] },
  { file: 'release-incident-recovery.md', title: 'Reprise après incident : rollback, roll-forward, hotfix', cat: 'CI/CD & livraison', level: 3, min: 45, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'cloud-rollback-decision' }, { kind: 'exercise', id: 'k8s-recovery-decision' }, { kind: 'exercise', id: 'incident-health-rollup' }] },
  // ── Kubernetes ──
  ...
  { file: 'k8s-config-probes.md', title: 'Kubernetes : configuration, secrets, probes et ressources', cat: 'Kubernetes', level: 3, min: 50, skills: ['cloud', 'secu'], practiceRefs: [{ kind: 'exercise', id: 'k8s-needs-probe' }, { kind: 'exercise', id: 'k8s-probe-role' }, { kind: 'exercise', id: 'k8s-oom-risk' }, { kind: 'exercise', id: 'k8s-secret-placement' }] },
  { file: 'k8s-troubleshooting.md', title: 'Kubernetes : diagnostiquer un incident', cat: 'Kubernetes', level: 3, min: 50, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'k8s-pod-phase' }, { kind: 'exercise', id: 'k8s-recovery-decision' }, { kind: 'exercise', id: 'k8s-config-drift' }, { kind: 'lab', id: 'kubernetes' }] },
  ...
  // ── Cloud, AWS, Azure & IaC ──
  { file: 'cloud-fundamentals.md', title: 'Cloud : concepts fondamentaux', cat: 'Cloud, AWS, Azure & IaC', level: 2, min: 50, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'cloud-service-model' }, { kind: 'exercise', id: 'cloud-classify-tier' }, { kind: 'lab', id: 'cloud-architecture' }] },
  { file: 'cloud-networking.md', title: 'Cloud : le réseau (VPC/VNet, subnets, pare-feu)', cat: 'Cloud, AWS, Azure & IaC', level: 3, min: 50, skills: ['cloud', 'secu'], practiceRefs: [{ kind: 'exercise', id: 'cloud-cidr-overlap' }, { kind: 'exercise', id: 'cloud-subnet-visibility' }, { kind: 'exercise', id: 'cloud-sg-port-range' }, { kind: 'exercise', id: 'cloud-public-exposure' }] },
  { file: 'cloud-compute-storage.md', title: 'Cloud : compute et stockage', cat: 'Cloud, AWS, Azure & IaC', level: 2, min: 50, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'cloud-compute-choice' }, { kind: 'exercise', id: 'cloud-storage-class' }, { kind: 'exercise', id: 'cloud-scaling-choice' }, { kind: 'exercise', id: 'cloud-scaling-kind' }] },
  ...
  { file: 'iac-fundamentals.md', title: 'Infrastructure as Code : les fondamentaux', cat: 'Cloud, AWS, Azure & IaC', level: 3, min: 50, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'k8s-config-drift' }] },
  { file: 'cloud-finops.md', title: 'FinOps : maîtriser le coût du cloud', cat: 'Cloud, AWS, Azure & IaC', level: 2, min: 45, skills: ['cloud', 'archi'], practiceRefs: [{ kind: 'exercise', id: 'cloud-rightsizing-savings' }, { kind: 'exercise', id: 'cloud-monthly-cost' }, { kind: 'exercise', id: 'cloud-error-budget' }] },
  // ── Production & DevOps ──
