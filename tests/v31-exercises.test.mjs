// CP7 (V31) — contrat des nouveaux exercices de raisonnement RAG / IA appliquée,
// vérifié par EXÉCUTION : schéma valide, ≥1 test public + ≥1 test privé,
// compétences connues, référence 100% verte, starter échoue ≥1 test public,
// aucune fuite de solution. Déterministe, PUR, local — aucun vrai embedding ni LLM.
...
const V31_EXERCISES = [
  'rag-chunking-overlap', 'rag-cosine-rank', 'rag-rrf-fusion',
  'rag-failure-locate', 'rag-structured-validate',
];

const tmp = mkdtempSync(join(tmpdir(), 'v31x-'));
let seq = 0;
...
test('V31 : les 5 exercices RAG existent et sont des fixtures valides', () => {
  for (const id of V31_EXERCISES) {
...
test('V31 : compétences connues, ≥1 test public et ≥1 test privé, référence présente', () => {
  for (const id of V31_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V31 : référence 100% verte, starter échoue ≥1 test public (exécuté)', async () => {
  for (const id of V31_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
