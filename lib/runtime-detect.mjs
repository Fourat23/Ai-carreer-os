...
// React/TSX : disponible ssi Node + typescript + react + react-dom sont résolubles.
function detectReact() {
  try {
    requireFromHere.resolve('typescript/package.json');
    const rv = requireFromHere('react/package.json')?.version ?? null;
    requireFromHere.resolve('react-dom/server');
    return { available: true, binary: process.execPath, version: rv ? `React ${rv} + TypeScript (local)` : 'React (local)' };
  } catch {
    return { available: false, binary: null, version: null, error: 'React / ReactDOM / TypeScript introuvables côté serveur.' };
  }
}
...
// Détecte le track Data/ML opt-in : le venv de projet .venv-ds doit exister ET
// numpy y être importable. Sinon indisponible → exercices sautés proprement.
function detectPythonDs() {
  const root = process.cwd();
  const candidates = [
    join(root, '.venv-ds', 'bin', 'python'),
    join(root, '.venv-ds', 'bin', 'python3'),
  ];
  for (const exe of candidates) {
    try {
      const out = execFileSync(exe, ['-c', 'import numpy, pandas, sklearn; print(numpy.__version__)'],
        { timeout: 15000, encoding: 'utf8' });
      const v = out.trim().split('\n').pop();
      return { available: true, binary: exe, version: `Python DS venv (numpy ${v} + pandas + scikit-learn)` };
    } catch { /* candidat suivant */ }
  }
  return { available: false, binary: null, version: null, error: 'Environnement Data/ML (.venv-ds) non provisionné. Voir scripts/v47-provision-ds-venv.sh.' };
}
