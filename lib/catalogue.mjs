...
const SYSTEMS_CLOUD_MODULE_SPECS = [
  ...
  ['scf-05-architecture-observability', 'Architecture & observabilité', 'Architecture 3-tiers/MVC, observabilité, cache et compromis — penser les systèmes.', 78, 81],
  ...
  ['scf-07-docker', 'Conteneurs & Docker (fondations)', 'Dockerisation : images/couches, Dockerfile, CMD/ENTRYPOINT, durcissement (non-root, read-only, network none), reproductibilité — rendre un service livrable et exploitable.', 320, 321],
];
...
export function buildCatalogue(program) {
  ...
  const systemsCloud = {
    ...
    goal: 'Poser des fondations opérationnelles solides — terminal/shell, Linux (permissions, processus, services), réseau (DNS/TCP/TLS/HTTP, SSH), diagnostic par couches, sécurité et observabilité — avant Docker et le cloud.',
  ...
  const catalogue = { technologies: TECHNOLOGIES, modules, tracks: [foundations, fullstack, backend, systemsCloud, ...announced] };
