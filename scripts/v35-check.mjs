#!/usr/bin/env node
// Gate V35 — lance : npm run v35:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V35 (cf.
// docs/architecture/v35-lessons-plan.json) : leçons de la chaîne RAG / IA appliquée durcies
// (embeddings, chunking, vector store, retrieval/reranking, rag-*, agents, sécurité IA).
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V35.md).
//
...
const planPath = R('docs/architecture/v35-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V35 (Track Architecture V2 + burn-down dette)');
  console.log('⚠️  plan v35-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V35 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V35 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
