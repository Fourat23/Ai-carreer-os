...
export async function runExercise(root, exercise, userFiles = {}) {
  ...
  try {
    ...
    const res = await execFileP(det.binary, adapter.buildArgs(adapter.harnessFile), {
      ...
      killSignal: 'SIGKILL',
      maxBuffer: adapter.maxOutputBytes,
      windowsHide: true,
      env: adapter.env(),
    });
    stdout = res.stdout ?? '';
    ...
    if (e.killed || e.signal === 'SIGKILL' || e.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER') {
      timedOut = true;
      error = `Exécution interrompue (délai de ${adapter.timeoutMs} ms dépassé ou sortie trop volumineuse).`;
    } else {
  ...
  return { attempt, stdout: userStdout.slice(0, adapter.maxOutputBytes), timedOut, error };
}
