...
function flatOf(track) {
  ...
  // V74 · CP2 · TENTATIVES D'EXERCICE. Le fait qui manquait : le CP0 a établi
  // qu'un exercice raté n'écrivait RIEN, parce que le produit persistait la
  // PROJECTION (la preuve) et jetait le FAIT (la tentative). Absentes du
  // disque, elles valent liste vide — jamais un état fabriqué, exactement
  // comme les tentatives de rappel.
  flat.exerciseAttempts = normalizeExerciseAttempts(track?.exerciseAttempts).slice(-MAX_EXERCISE_ATTEMPTS);
  // ── V75 · CP10 · TENTATIVES DE TRANSFERT ──
  //
  // **Cette fonction est une LISTE BLANCHE**, et c'est le piège qu'elle tend :
  // un champ absent d'ici est écrit par le moteur puis **silencieusement perdu
  // à la persistance**. Le fait existait, les tests unitaires passaient, et le
  // disque ne gardait rien.
  //
  // Trouvé en traversant la chaîne réelle en HTTP (§10.6), pas en relisant le
  // code — et deux champs manquaient, pas un.
  flat.transferAttempts = normalizeTransferAttempts(track?.transferAttempts).slice(-MAX_TRANSFER_ATTEMPTS);
  // V76 · CP7 — les aides consultées. Ajoutées aux DEUX listes blanches en même
  // temps : c'est le défaut P7 de V75, où `curriculumPause` a vécu trois
  // checkpoints dans une seule des deux et n'a jamais atteint le disque.
  flat.hintViews = normalizeHintViews(track?.hintViews).slice(-MAX_HINT_VIEWS);
...
export function emptyFlat() {
  return { startDate: null, days: {}, skills: {}, weeklyReviews: {}, monthlyReviews: {}, evidence: [], recallAttempts: [], exerciseAttempts: [], transferAttempts: [], hintViews: [] };
}
...
export function activeTrackProgress(v3) {
  ...
  flat.exerciseAttempts = normalizeExerciseAttempts(t.exerciseAttempts);
  // ── V75 · CP10 — LA SECONDE LISTE BLANCHE ──
  //
  // `flatOf` filtre à l'ÉCRITURE, celle-ci filtre à la LECTURE. Les deux
  // doivent connaître chaque fait, et rien ne le rappelle : un champ oublié ici
  // est écrit sur le disque puis invisible au rechargement. Le test
  // « aucun fait n'est perdu par le disque » garde désormais les deux.
  flat.transferAttempts = normalizeTransferAttempts(t.transferAttempts);
  flat.hintViews = normalizeHintViews(t.hintViews);
