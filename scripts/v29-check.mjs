#!/usr/bin/env node
// Gate V29 — lance : npm run v29:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V29 (cf.
// docs/architecture/v29-lessons-plan.json) : NOUVELLES leçons Frontend/React, Data/SQL,
// Software Engineering ET leçons HISTORIQUES corrigées (dette P0 + domaines cibles).
// Mêmes contrôles que v27/v28 : on-ramp débutant AVANT l'objectif, prérequis explicités
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V29.md).
//
...
const planPath = R('docs/architecture/v29-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V29 (socle + Frontend/React + Data/SQL + SE) ──');
  console.log('⚠️  plan v29-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V29 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V29 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
