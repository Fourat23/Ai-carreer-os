// CP4 (V37) — contrat des exercices Web ajoutés, vérifié par EXÉCUTION via
...
const V37_EXERCISES = ['css-specificity-order', 'css-box-size', 'dom-event-delegation'];
...
test('V37 : les exercices frontend existent et sont des fixtures valides', () => {
  for (const id of V37_EXERCISES) {
...
test('V37 : compétences connues, ≥1 test public et ≥1 test privé, référence présente', () => {
  for (const id of V37_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V37 : référence 100% verte, starter échoue ≥1 test public (exécuté)', async () => {
  for (const id of V37_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V37 : exercices reliés à un jour réel (reachables)', () => {
  const dayEx = JSON.parse(read('data/day-exercises.json'));
  const reachable = new Set(Object.values(dayEx).flat());
  for (const id of V37_EXERCISES) assert.ok(reachable.has(id), `${id} : relié à un jour`);
});
