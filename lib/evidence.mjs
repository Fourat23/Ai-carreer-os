...
export function niveauDePreuve(sourceType, validation) {
  const plafond = NIVEAU_MAX_PAR_SOURCE[sourceType] ?? 'DECLARED';
  ...
  let brut = 'OBSERVED';
  if (statut === 'passed') {
    // `self` est une DÉCLARATION de l'apprenant, même quand elle dit « réussi ».
    brut = kind === 'self' ? 'DECLARED' : 'VALIDATED';
  } else if (statut === null) {
    brut = 'DECLARED';
  }
