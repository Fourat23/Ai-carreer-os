...
import { migrateToV7, normalizeMissionsMap, normaliserLesFaits } from './progress-store.mjs';
...
export function validateStrict(src) {
  ...
  // État des missions (V18) : préservé au backup, borné et sûr (jamais de secret
  // ni de test privé — ce ne sont que les livrables de l'apprenant + leur statut).
  const progress = { startDate, days, skills, weeklyReviews: wr.value, monthlyReviews: mr.value };
  const missions = normalizeMissionsMap(src.missions);
  if (missions) progress.missions = missions;

  // ── V77 · CP3 — LA QUATRIÈME LISTE BLANCHE ──
  //
  // Cette fonction reconstruit la progression importée champ par champ, et elle
  // n'a jamais connu les faits introduits depuis V66. Conséquence **mesurée**,
  // avant correction, sur un aller-retour export → import :
  //
  //   recallAttempts 1 → 0 · hintViews 1 → 0 · usageEvents 1 → 0
  //
  // L'export était fidèle, la restauration muette : les tentatives de rappel,
  // d'exercice et de transfert, les aides consultées et les preuves canoniques
  // sans journée (transfert, mission) ne survivaient pas à leur propre
  // sauvegarde. Le registre était alors reconstruit depuis `days[*].evidence`,
  // ce qui ressemblait à une restauration réussie.
  //
  // `normaliserLesFaits` reste une liste blanche bornée : un fichier importé n'y
  // gagne aucun droit d'écrire ce que les normaliseurs refusent. Le seul
  // changement est qu'on cesse de **jeter** ce qu'ils acceptent.
  Object.assign(progress, normaliserLesFaits(src, { daysPourHeritage: days }));
 ...
 * + workspaces du Laboratoire. Aucune I/O (les workspaces sont fournis). Le
 * runtime d'un exercice (Node / Python / TypeScript) n'est PAS persisté ici :
 * il provient de la définition de l'exercice (source unique) au moment de la
 * restauration ; on ne sauvegarde que le CONTENU éditable des fichiers.
 * @param {object} v3  { schemaVersion:3, activeTrackId, tracks }
 * @param {Record<string,{files:Record<string,string>}>} workspaces
 */
