...
import { normalizeDayAttempt } from './event-model.mjs';
...
function normalizeAttempts(a) {
  ...
  const history = Array.isArray(a.history)
    ? a.history.slice(0, MAX_ATTEMPTS).filter(isObj).map((h) => ({
        ...
        // V75 · CP2 — additifs. Une entrée écrite avant ce contrat n'a ni
        // provenance ni version : elle reçoit `legacy` / `1`, ce qui la rend
        // VISIBLE plutôt que muette (G9 de V74).
        provenance: isObj(h.provenance) && typeof h.provenance.producer === 'string'
          ? { producer: s(h.provenance.producer, 60), method: s(h.provenance.method, 80), note: s(h.provenance.note, 200) }
          : { producer: 'legacy', method: '', note: '' },
        schemaVersion: Number.isInteger(h.schemaVersion) ? h.schemaVersion : 1,
      }))
    : [];
...
// V75 · CP2 — D3 payée : l'événement est TYPÉ et tracé, pas supprimé.
//
// Le CP0 a mesuré 1 producteur, 0 consommateur réel, un `outcome` en chaîne
// libre, ni provenance ni idempotence. Supprimer le champ aurait été une
// migration destructive — la progression d'un apprenant réel peut déjà en
// contenir, et *un fait qu'on ne lit plus n'est pas un fait qu'on a le droit
// d'effacer*.
//
// Ce qui change : vocabulaire FERMÉ (`ATTEMPT_OUTCOMES`), provenance présente,
// version de schéma. Ce qui ne change pas : la forme `{ at, outcome, summary }`
// reste lisible, et les entrées écrites avant ce contrat traversent intactes.
export function recordAttempt(day, { at, outcome = '', summary = '', provenance = null } = {}) {
  const d = normalizeDay(day);
  const norm = normalizeDayAttempt(
    { at, outcome, summary, provenance },
    { now: isoOrNull(at) ?? new Date(0).toISOString(), legacy: true },
  );
  const entry = {
    at: norm.at, outcome: norm.outcome, summary: norm.summary,
    provenance: norm.provenance, schemaVersion: norm.schemaVersion,
  };
