...
- **Étage retrieval (programmatique, sans LLM)** : rappel@k (le bon chunk est-il dans le top-k ?) et **MRR**, pour *Mean Reciprocal Rank*, le **rang réciproque moyen** — à quel rang le bon passage est-il remonté ? On note l'inverse de ce rang (1 s'il est premier, 0,5 s'il est deuxième, 0,33 s'il est troisième), puis on fait la moyenne sur toutes les questions. C'est la seule des deux qui distingue « bon document en position 1 » de « bon document en position 5 », et c'est pour cela qu'elle voit ce que le rappel ne peut pas voir : un reranking ne change pas *quels* documents sont présents, il change leur **ordre**. Rapide, fiable, gratuit — la métrique au meilleur rendement du domaine. C'est ici que se diagnostiquent 80 % des échecs. (Attention au sigle : en gestion d'entreprise, MRR désigne le revenu récurrent mensuel — aucun rapport.)
