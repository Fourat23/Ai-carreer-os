...
(function(){
  ...
  function ser(v,d){ d=d||0; try{
    ...
    if(Array.isArray(v)) return '['+v.slice(0,20).map(function(x){return ser(x,d+1);}).join(', ')+(v.length>20?', …':'')+']';
    var ks=Object.keys(v).slice(0,20), out=[];
    for(var i=0;i<ks.length;i++){ out.push(ks[i]+': '+ser(v[ks[i]],d+1)); }
