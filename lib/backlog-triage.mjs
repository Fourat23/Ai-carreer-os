 ...
 * @param {number} [o.capaciteActive]  nombre d'unités que la séance peut prendre
 * @param {string} o.now
 */
  ...
  const notions = enRetard.map((f) => {
    ...
    if (bloquants) {
      return {
        ...base,
        classe: 'PARKED',
        raison: `on reprend d'abord « ${bloquants[0]} », dont cette notion dépend`,
        conditionDeRetour: `quand « ${bloquants[0]} » sera repris`,
