#!/usr/bin/env node
// Gate V31 — lance : npm run v31:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V31 (cf.
// docs/architecture/v31-lessons-plan.json) : leçons de la chaîne RAG / IA appliquée durcies
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V31.md).
//
...
const planPath = R('docs/architecture/v31-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V31 (RAG & IA appliquée + Curriculum Graph) ──');
  console.log('⚠️  plan v31-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V31 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V31 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
