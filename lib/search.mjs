 ...
 */
export function buildIndex(program, catalogue = null, exercises = null, missions = null, pipelines = null, topologies = null, manifests = null, scenarios = null, playbooks = null, glossaryEntries = null) {
  const items = [];
  ...
  for (const p of playbooks ?? []) {
    if (!p || typeof p.id !== 'string') continue;
    push('playbook', p.situation ?? p.title ?? p.id, `/security#playbook-${p.id}`, 'Que faire dans ce cas ?',
      `que faire playbook procedure incident ${p.situation ?? p.title ?? ''} ${p.domain ?? ''}`);
  }
...
const TYPE_ORDER = { command: 0, day: 1, exercise: 2, mission: 2, pipeline: 2, topology: 2, manifest: 2, scenario: 2, playbook: 2, track: 3, module: 4, week: 5, month: 6, skill: 7, technology: 8, glossary: 9, project: 10, lesson: 11, page: 12 };
