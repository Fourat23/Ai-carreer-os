// CP3+ (V34) — contrat des exercices de mécanique d'agent, vérifié par EXÉCUTION :
...
test('V34 : les exercices d\'agent existent et sont des fixtures valides', () => {
  for (const id of V34_EXERCISES) {
...
test('V34 : compétences connues, ≥1 test public et ≥1 test privé, référence présente, SIMULATION', () => {
  for (const id of V34_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V34 : référence 100% verte, starter échoue ≥1 test public (exécuté)', async () => {
  for (const id of V34_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
