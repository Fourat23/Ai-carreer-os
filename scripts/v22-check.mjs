#!/usr/bin/env node
// Gate V22 — lance : npm run v22:check
//
// 1. Valide TOUTES les topologies (data/topologies/*.json) contre les données
//    réelles (jours, parcours dérivés du catalogue, compétences) + anti-fuite de
//    la vue publique (aucun secret).
...
const plan = JSON.parse(readFileSync(R('docs/architecture/v22-enrichment-plan.json'), 'utf8'));
...
// 1. Topologies valides + anti-fuite + analysables.
const dir = R('data/topologies');
const topos = existsSync(dir) ? readdirSync(dir).filter((f) => f.endsWith('.json')) : [];
const ids = new Set();
for (const f of topos) {
  const t = JSON.parse(readFileSync(join(dir, f), 'utf8'));
  const v = validateTopology(t, ctx);
  if (!v.ok) errors.push(`topologie ${f} invalide : ${v.errors.join(' ; ')}`);
  if (ids.has(t.id)) errors.push(`topologie : id dupliqué « ${t.id} »`);
  ids.add(t.id);
  const blob = JSON.stringify(publicTopologyView(t));
  if (/sk-[A-Za-z0-9]{8,}|ghp_[A-Za-z0-9]{8,}|AKIA[0-9A-Z]{12,}/.test(blob)) errors.push(`topologie ${f} : fuite de secret dans la vue publique`);
  try { analyzeTopology(t); } catch (e) { errors.push(`topologie ${f} : analyse en échec — ${e.message}`); }
}
...
console.log('── Gate V22 (Cloud Topology Lab & Deployment) ──');
console.log(`Baseline            : ${baseRef}${baseOk ? '' : ' (INTROUVABLE)'}`);
console.log(`Topologies validées : ${topos.length}`);
...
console.log('\n✅ V22 valide : topologies cohérentes, aucune dérive hors périmètre, profondeur minimale présente.');
