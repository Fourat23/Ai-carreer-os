#!/usr/bin/env node
// Gate V33 — lance : npm run v33:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V33 (cf.
// docs/architecture/v33-lessons-plan.json) : leçons de la chaîne RAG / IA appliquée durcies
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V33.md).
//
...
const planPath = R('docs/architecture/v33-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V33 (ML → DL → Transformers → LLMOps) ──');
  console.log('⚠️  plan v33-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V33 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V33 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
