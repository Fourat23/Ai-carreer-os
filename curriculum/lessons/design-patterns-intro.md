...
Le piège vient de ce que les exemples canoniques ont été écrits pour le Java des années 1990, qui n'avait aucun moyen de passer un comportement en argument : les classes y sont un contournement de cette limite, pas le pattern lui-même. (Java l'a depuis : les lambdas et les références de méthode sont arrivées avec Java 8, en 2014.) En JavaScript, où une fonction est une valeur depuis toujours, la moitié des patterns du livre tiennent en une ligne — c'est un progrès du langage, pas une absence de conception.
