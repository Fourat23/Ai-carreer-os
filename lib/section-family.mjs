...
export function annotateDayHtml(html) {
  ...
  const a11y = html
    .replace(/<input([^>]*?)type="checkbox"([^>]*?)>/g,
      (m, a, b2) => (/aria-hidden/.test(m) ? m : `<input${a}type="checkbox"${b2} aria-hidden="true" tabindex="-1">`))
    .replace(/<pre(?![^>]*tabindex)([^>]*)>/g,
      (_m, attrs) => `<pre${attrs} tabindex="0" role="region" aria-label="Bloc de code (défilement horizontal possible)">`);
