...
import { normalizeExerciseAttempt, normalizeExerciseAttempts, attemptKey } from './exercise-attempt.mjs';
import { normalizeHintView, normalizeHintViews, hintViewKey } from './hint-view.mjs';
import { normalizeUsageEvent, normalizeUsageEvents, usageEventKey } from './usage-event.mjs';
import {
  normalizeMissionSubmission, normalizeMissionSubmissions, missionSubmissionKey,
} from './mission-submission.mjs';
import {
  normalizeArtifactAnalysis, normalizeArtifactAnalyses, artifactAnalysisKey,
} from './artifact-analysis.mjs';
import {
  normalizeAssessmentAttempt, normalizeAssessmentAttempts, assessmentAttemptKey,
  // Alias explicite : `transfer-attempt.mjs` exporte une fonction du même nom,
  // pour le même problème sur une autre surface. Les confondre écarterait des
  // faits qui n'ont rien à voir.
  estUnRejeu as estUnRejeuDeDiagnostic,
} from './assessment-attempt.mjs';
import {
  normalizeTransferAttempt, normalizeTransferAttempts, transferAttemptKey, estUnRejeu,
} from './transfer-attempt.mjs';
...
export const COMMANDS = [
  ...
  'RECORD_RECALL',
  // rétention (V74 · CP2) — la tentative d'exercice devient un FAIT. Le CP0 a
  // établi qu'un exercice raté n'écrivait rien : le produit persistait la
  // projection (la preuve) et jetait le fait. Cette commande écrit le fait,
  // que la tentative réussisse OU NON.
  'RECORD_EXERCISE_ATTEMPT',
  // V75 · CP10 — `TransferAttempt`, le septième fait. Le CP9 a rendu les défis
  // atteignables et le produit n'écrivait toujours RIEN quand un transfert
  // échoue : même dissymétrie que celle corrigée au CP2 de V74 pour les
  // exercices. Cette commande écrit le fait, que le transfert réussisse OU NON.
  'RECORD_TRANSFER_ATTEMPT',
  // V76 · CP7 — la lecture d'une aide est un fait, au même titre que la
  // tentative qui l'a provoquée.
  'RECORD_HINT_VIEW',
  // V77 · CP3 — un USAGE observé. Le CP0 a mesuré que le terminal exécute
  // vraiment mais que ses tâches ne portent AUCUN critère de réussite
  // pédagogique : leurs arguments sont des énumérations fermées. Cette commande
  // écrit donc ce qui a eu lieu, et **jamais** une réussite. Elle ne produit
  // aucune preuve et n'alimente aucun moteur ; c'est sa raison d'être.
  'RECORD_USAGE_EVENT',
  // V77 · CP4 — la soumission d'un diagnostic devient un FAIT. Le CP4 a mesuré
  // que cinq échecs successifs ne laissaient qu'UNE trace, et que c'était la
  // PREMIÈRE : la clé de preuve ignore le score, donc une amélioration sous le
  // seuil était refusée comme doublon. Même cause qu'en V74 · CP2 — persister
  // la projection et jeter le fait.
  'RECORD_ASSESSMENT_ATTEMPT',
  // V77 · CP5 — le livrable rendu devient un fait, et il dit par quel MODE il a
  // été constaté. C'est ce qui permet à la preuve de mission de cesser de dire
  // `passed` sans perdre l'information de ce qui a réellement été fait.
  'RECORD_MISSION_SUBMISSION',
  // V77 · CP7 — l'artefact analysé. Quatre surfaces rendaient un résultat
  // substantiel et n'écrivaient rien. Le fait n'existe QUE si l'apprenant a
  // posté un artefact : analyser la fixture fournie par le produit n'est pas un
  // travail, c'est une page qui s'affiche.
  'RECORD_ARTIFACT_ANALYSIS',
  // preuves
  'ADD_EVIDENCE', 'REMOVE_EVIDENCE',
  // hors journée
  'SET_SKILL', 'SET_WEEKLY_REVIEW', 'SET_MONTHLY_REVIEW',
  // V75 · CP7 — `PAUSED_CURRICULUM` (§1.5 du contrat de récupération) : un
  // CHOIX DE L'APPRENANT, que le moteur ne peut qu'enregistrer. Aucune règle
  // n'active cette commande toute seule ; elle n'existe que parce qu'une
  // personne a cliqué. C'est la différence entre recommander et imposer.
  'SET_CURRICULUM_PAUSE',
];

import { normalizeWeeklyReview } from './event-model.mjs';
...
function canonicalSourceFor(cmd, submission) {
  ...
  // ── V76 · CP11 — DEUX NOMS POUR DEUX CHOSES DIFFÉRENTES ──
  //
  // `evidenceId` nomme la PREUVE DE JOURNÉE ; le laboratoire y met
  // `lab-<exerciseId>`, le même identifiant que `recordExerciseSuccess`, et les
  // deux chemins convergent correctement de ce côté-là.
  //
  // Le même champ servait aussi de `sourceId` à la preuve CANONIQUE. Or
  // `recordExerciseSuccess` nomme cette source `<exerciseId>`, sans préfixe.
  // Deux noms pour le même fait ⇒ deux entrées au registre ⇒ **une seule
  // réussite fournissait DEUX « sources distinctes »**, c'est-à-dire exactement
  // ce que la règle de consolidation exige pour promouvoir une compétence à
  // `reinforced`. Mesuré au CP11 : 2 preuves, 2 sources, 2 contacts de
  // rétention pour un seul exercice résolu une fois.
  //
  // `canonicalSourceId` nomme donc LE FAIT, séparément de la preuve de journée.
  // Absent, le comportement est celui d'avant.
  const nomDuFait = validId(cmd.canonicalSourceId, 64) ?? cmd.evidenceId;
  if (kind === 'exercise-tests') return { sourceType: 'exercise', sourceId: nomDuFait ?? submission.stepId };
  if (kind === 'assessment-grade') return { sourceType: 'assessment', sourceId: nomDuFait ?? submission.stepId };
...
function canonicalEvidenceFor(cmd, submission, dayNum, nowIso) {
  ...
  return makeEvidence({
    ...
    // ── V75 · CP4 — LE SECOND PRODUCTEUR DE PREUVES VOIT AUSSI LES CONCEPTS ──
    //
    ...
    //
    // Le CP3 n'avait instrumenté que la première. La seconde repartait sans
    // concept et retombait sur le rattachement PAR JOURNÉE — c'est-à-dire
    // qu'elle recréditait les 3 leçons médianes de la journée, annulant
    // exactement le gain du CP3 pour le même exercice. Le champ est additif :
    // absent, le comportement est celui d'avant.
    conceptIds: Array.isArray(cmd.conceptIds) ? cmd.conceptIds : [],
...
function cmdRecordRecall(progress, cmd, nowIso) {
  ...
  const attempt = normalizeAttempt({
    ...
    // V77.1 · CP4 — qui a constaté ce rappel. La station de rappel du produit
    // est le producteur nominal ; un appelant qui n'en fournit aucune obtient
    // `legacy`, visible dans l'export plutôt que muet.
    provenance: cmd.provenance ?? { producer: 'recall-station', method: 'auto-report' },
  });
...
/**
 * V74 · CP2 — enregistrement d'une TENTATIVE D'EXERCICE.
 *
 * Contrairement à `RECORD_RECALL`, l'issue n'est PAS fournie par l'appelant :
 * elle est dérivée de `passed`/`total` par le normaliseur. Un appelant qui
 * mentirait sur l'issue n'aurait aucun effet — les compteurs sont le fait.
 *
 * Idempotence par CLÉ MÉTIER (§3.6 du contrat gelé) : `exerciseId` + seconde +
 * `passed/total`. Un rejeu réseau ou un double-clic ne crée pas deux faits.
 */
function cmdRecordExerciseAttempt(progress, cmd, nowIso) {
  const attempt = normalizeExerciseAttempt({
    ...
    dayRefs: cmd.dayRefs,
    correctionSeen: cmd.correctionSeen,
    provenance: cmd.provenance ?? { producer: 'lab-runner', method: 'sandbox-tests' },
  });
  if (!attempt) return fail('INVALID_EXERCISE_ATTEMPT', 'Tentative d’exercice invalide.');
  const existantes = normalizeExerciseAttempts(progress.exerciseAttempts ?? []);
  const cle = attemptKey(attempt);
  if (existantes.some((a) => attemptKey(a) === cle)) {
    // Déjà enregistrée : succès sans écriture. Un fait idempotent ne doit ni
    // échouer ni se dupliquer.
    return { ok: true, progress, effects: [] };
  }
  return {
    ok: true,
    progress: { ...progress, exerciseAttempts: [...existantes, attempt] },
    effects: ['exercise-attempt:recorded'],
  };
}

/**
 * ── V75 · CP10 — LA TENTATIVE DE TRANSFERT ────────────────────────────────
 *
 * Écrite systématiquement, succès comme échec. Sur un défi de transfert,
 * l'échec est l'information la PLUS utile : c'est lui qui dit « la notion tient
 * chez elle et cède ailleurs », et c'est exactement ce qu'aucun exercice ne
 * peut révéler.
 *
 * §10.4 — **une reprise est un fait NEUF.** La tentative précédente n'est
 * jamais écrasée ; `retryOf` porte sa clé, et l'historique complet
 * (`failure → partial → success`) reste reconstructible.
 *
 * §10.5 — **un rejeu réseau n'est pas une tentative humaine.** Voir la fenêtre
 * déclarée dans `lib/transfer-attempt.mjs`.
 */
function cmdRecordTransferAttempt(progress, cmd, nowIso) {
  const existantes = normalizeTransferAttempts(progress.transferAttempts ?? []);
  const duDefi = existantes.filter((a) => a.challengeId === cmd.challengeId);
  const derniere = duDefi.length ? duDefi[duDefi.length - 1] : null;

  const attempt = normalizeTransferAttempt({
    challengeId: cmd.challengeId,
    at: nowIso,
    conceptIds: cmd.conceptIds,
    competencyIds: cmd.competencyIds,
    passed: cmd.passed,
    total: cmd.total,
    startedAtDeclare: cmd.startedAtDeclare,
    evidenceId: cmd.evidenceId,
    // La reprise se CHAÎNE d'office sur la dernière tentative du même défi :
    // l'appelant n'a pas à la connaître, et ne peut donc pas la fausser.
    retryOf: derniere ? transferAttemptKey(derniere) : null,
    sourceRef: cmd.sourceRef,
    empreinte: cmd.empreinte,
    provenance: cmd.provenance ?? { producer: 'transfer-grader', method: 'transfer-challenge' },
  }, { now: nowIso });

  if (!attempt) return fail('INVALID_TRANSFER_ATTEMPT', 'Tentative de transfert invalide.');

  const cle = transferAttemptKey(attempt);
  if (existantes.some((a) => transferAttemptKey(a) === cle)) {
    return { ok: true, progress, effects: ['noop:transfer-attempt:duplicate'] };
  }
  // Rejeu réseau : même défi, mêmes réponses, même issue, dans la fenêtre.
  // Idempotent — succès sans écriture, comme pour une clé déjà vue.
  if (estUnRejeu(attempt, derniere)) {
    return { ok: true, progress, effects: ['noop:transfer-attempt:replay'] };
  }

  return {
    ok: true,
    progress: { ...progress, transferAttempts: [...existantes, attempt] },
    effects: ['transfer-attempt:recorded'],
  };
}

/**
 * ── V76 · CP7 — UNE AIDE CONSULTÉE EST UN FAIT ──────────────────────────
 *
 * Le CP0 avait mesuré que rien n'enregistrait la lecture d'une aide : une
 * réussite après trois indices était indiscernable d'une réussite immédiate,
 * alors que le contrat gelé (§1.12) exige que « la provenance le montre ».
 *
 * **Ce fait ne dévalue rien.** Il décrit. Le contrat interdit d'en faire une
 * pénalité, et `provenanceDeLaReussite` rend `reussite: true` en toutes
 * circonstances.
 *
 * Idempotent par clé métier : deux affichages du même indice à la même seconde
 * ne sont pas deux consultations.
 */
function cmdRecordHintView(progress, cmd, nowIso) {
  const vue = normalizeHintView({
    exerciseId: cmd.exerciseId,
    action: cmd.action,
    niveau: cmd.niveau,
    declenchee: cmd.declenchee,
    provenance: cmd.provenance ?? { producer: 'lab-remediation', method: 'echelle-aide' },
  }, { now: nowIso });
  if (!vue) return fail('INVALID_HINT_VIEW', 'Consultation d’aide invalide.');

  const existantes = normalizeHintViews(progress.hintViews);
  const cle = hintViewKey(vue);
  if (existantes.some((v) => hintViewKey(v) === cle)) {
    return { ok: true, progress, effects: ['noop:hint-view:duplicate'] };
  }
  return {
    ok: true,
    progress: { ...progress, hintViews: [...existantes, vue] },
    effects: ['hint-view:recorded'],
  };
}

/**
 * ── V77 · CP3 — UN USAGE OBSERVÉ, ET RIEN DE PLUS ───────────────────────
 *
 * La commande la plus pauvre du moteur, et c'est voulu. Elle n'a **aucune**
 * branche de réussite : ni `passed`, ni `score`, ni preuve, ni effet sur la
 * compétence, la rétention ou la récupération. Le CP0 a mesuré pourquoi — les
 * trois tâches du terminal ne portent aucun critère pédagogique et leurs
 * arguments sont des énumérations fermées :
 *
 *   > « Un apprenant qui choisit `-la` parmi trois options valides n'a rien
 *   >   démontré. »
 *
 * Écrire `TerminalAttempt(success = true)` aurait fabriqué un verdict là où le
 * produit n'observe qu'un usage. C'est la ligne que le contrat gelé interdit
 * nommément, et c'est exactement le piège de Goodhart que V77 doit éviter :
 * améliorer la couverture en inventant du sens.
 *
 * Ce qui reste est honnête et utile à une seule question, celle que V78 posera
 * d'humains réels : *« ce participant est-il allé au terminal ? »*
 *
 * Idempotent par clé métier : un rejeu réseau ne compte pas deux fois.
 */
function cmdRecordUsageEvent(progress, cmd, nowIso) {
  ...
  if (!ev) return fail('INVALID_USAGE_EVENT', 'Événement d’usage invalide.');

  const existants = normalizeUsageEvents(progress.usageEvents);
  const cle = usageEventKey(ev);
  if (existants.some((e) => usageEventKey(e) === cle)) {
    return { ok: true, progress, effects: ['noop:usage-event:duplicate'] };
  }
  return {
    ok: true,
    progress: { ...progress, usageEvents: [...existants, ev] },
    effects: ['usage-event:recorded'],
  };
}

/**
 * ── V77 · CP4 — UNE SOUMISSION EST UN FAIT, PAS UN DOUBLON ──────────────
 *
 * Mesuré avant d'écrire : sept soumissions humaines sur le même diagnostic
 * laissaient **2 preuves et 0 tentative**, et sur une autre progression
 * `0/5 → 1/5 → 4/5` laissait `0/5` puis `4/5` — le `1/5` refusé comme doublon.
 * La clé de preuve ignore le score ; celle d'une tentative ne l'ignore pas,
 * parce qu'une tentative identifie un ACTE et non son résultat.
 *
 * Deux protections, et elles ne disent pas la même chose :
 *
 *   · la **clé métier** (diagnostic + seconde + empreinte des réponses) écarte
 *     la même requête livrée deux fois ;
 *   · `estUnRejeu` écarte la séquence « corriger puis conserver » de
 *     l'interface — mêmes réponses, même issue, moins de dix secondes. **Une
 *     seule pensée, deux requêtes.** Sans elle, chaque diagnostic conservé
 *     compterait double, ce qui gonflerait la courbe d'échecs au lieu de la
 *     révéler.
 *
 * Ce qu'aucune des deux ne fait : fusionner cinq soumissions humaines
 * distinctes. C'est exactement ce que le produit faisait, et c'est le défaut
 * que ce checkpoint corrige.
 */
function cmdRecordAssessmentAttempt(progress, cmd, nowIso) {
  const tentative = normalizeAssessmentAttempt({
    ...
    evidenceId: cmd.evidenceId,
    sourceRef: cmd.sourceRef,
    provenance: cmd.provenance ?? { producer: 'assessment-grader', method: 'assessment-grade' },
  }, { now: nowIso });
  if (!tentative) return fail('INVALID_ASSESSMENT_ATTEMPT', 'Soumission de diagnostic invalide.');

  const existantes = normalizeAssessmentAttempts(progress.assessmentAttempts);
  const cle = assessmentAttemptKey(tentative);
  if (existantes.some((a) => assessmentAttemptKey(a) === cle)) {
    return { ok: true, progress, effects: ['noop:assessment-attempt:duplicate'] };
  }
  const derniere = [...existantes].reverse().find((a) => a.assessmentId === tentative.assessmentId) ?? null;
  if (estUnRejeuDeDiagnostic(tentative, derniere)) {
    return { ok: true, progress, effects: ['noop:assessment-attempt:replay'] };
  }
  return {
    ok: true,
    progress: { ...progress, assessmentAttempts: [...existantes, tentative] },
    effects: ['assessment-attempt:recorded'],
  };
}

/**
 * ── V77 · CP5 — UN LIVRABLE RENDU, ET LE MOYEN DE SON CONSTAT ───────────
 *
 * Le fait ne dit pas que le travail est bon. Il dit qu'il a été rendu, et par
 * quel MODE il a été constaté — auto-vérifié, structurellement conforme, ou
 * signé par l'apprenant. Le niveau se dérive du mode, jamais du résultat : une
 * revue signée avec conviction reste une déclaration.
 *
 * C'est ce fait qui permet à la preuve de mission de cesser de dire `passed`
 * **sans rien perdre** : ce que le mot `passed` recouvrait abusivement est
 * désormais écrit là où c'est vrai.
 */
function cmdRecordMissionSubmission(progress, cmd, nowIso) {
  const fait = normalizeMissionSubmission({
    missionId: cmd.missionId,
    deliverableId: cmd.deliverableId,
    mode: cmd.mode,
    statut: cmd.statut,
    structureOk: cmd.structureOk,
    manques: cmd.manques,
    tailleContenu: cmd.tailleContenu,
    provenance: cmd.provenance ?? { producer: 'mission-engine', method: 'mission-deliverables' },
  }, { now: nowIso });
  if (!fait) return fail('INVALID_MISSION_SUBMISSION', 'Livrable de mission invalide.');

  const existants = normalizeMissionSubmissions(progress.missionSubmissions);
  const cle = missionSubmissionKey(fait);
  if (existants.some((s) => missionSubmissionKey(s) === cle)) {
    return { ok: true, progress, effects: ['noop:mission-submission:duplicate'] };
  }
  return {
    ok: true,
    progress: { ...progress, missionSubmissions: [...existants, fait] },
    effects: ['mission-submission:recorded'],
  };
}

/**
 * ── V77 · CP7 — UN ARTEFACT ANALYSÉ, ET AUCUN VERDICT ───────────────────
 *
 * L'apprenant rédige vraiment quelque chose — un manifeste, une topologie, une
 * architecture, un scénario — et le produit l'analyse vraiment. Mais il ne sait
 * pas le JUGER : ses diagnostics signalent ce qu'il sait reconnaître.
 *
 *   > Un compte de diagnostics n'est pas un verdict, et `0 diagnostic` ne veut
 *   > pas dire « c'est juste ».
 *
 * La commande n'a donc aucune branche de réussite, et le fait n'existe pas
 * quand l'artefact vient de la fixture : compter l'affichage d'une page comme
 * du travail est la façon la plus facile de gonfler une couverture.
 */
function cmdRecordArtifactAnalysis(progress, cmd, nowIso) {
  const fait = normalizeArtifactAnalysis({
    surface: cmd.surface,
    artifactId: cmd.artifactId,
    artefactFourni: cmd.artefactFourni,
    diagnostics: cmd.diagnostics,
    parSeverite: cmd.parSeverite,
    dimensions: cmd.dimensions,
    empreinte: cmd.empreinte,
    tailleArtefact: cmd.tailleArtefact,
    provenance: cmd.provenance ?? { producer: 'artifact-analyzer', method: 'analyze' },
  }, { now: nowIso });
  if (!fait) return fail('INVALID_ARTIFACT_ANALYSIS', 'Analyse d’artefact invalide.');

  const existants = normalizeArtifactAnalyses(progress.artifactAnalyses);
  const cle = artifactAnalysisKey(fait);
  if (existants.some((a) => artifactAnalysisKey(a) === cle)) {
    return { ok: true, progress, effects: ['noop:artifact-analysis:duplicate'] };
  }
  return {
    ok: true,
    progress: { ...progress, artifactAnalyses: [...existants, fait] },
    effects: ['artifact-analysis:recorded'],
  };
}

function cmdOutsideDay(progress, cmd, nowIso) {
  switch (cmd.type) {
    // ── V75 · CP7 — LA PAUSE DU CURRICULUM ────────────────────────────────
    //
    // §1.5 du contrat : *« un choix de l'apprenant, jamais du moteur ».* Le
    // produit peut le PROPOSER (`CRITICAL`, CP6) ; seule cette commande
    // l'enregistre, et rien ne l'appelle automatiquement.
    //
    // **La pause ne touche RIEN d'autre.** Elle n'efface aucune notion due, ne
    // décale aucune échéance, ne change aucun état de rétention : ce serait
    // `R2` — supprimer artificiellement des notions dues — déguisé en
    // fonctionnalité de confort. Elle ne fait qu'inscrire un fait daté : *« à
    // cette date, j'ai décidé de suspendre le nouveau contenu. »*
    case 'SET_CURRICULUM_PAUSE': {
      const paused = cmd.paused === true;
      const actuelle = isObj(progress.curriculumPause) ? progress.curriculumPause : null;
      if (Boolean(actuelle?.paused) === paused) {
        return { ok: true, progress, effects: [`noop:curriculum-pause:${paused ? 'on' : 'off'}`] };
      }
      const provenance = isObj(cmd.provenance) ? cmd.provenance : null;
      return {
        ok: true,
        progress: {
          ...progress,
          curriculumPause: {
            paused,
            // `since` porte le DÉBUT de la pause en cours ; reprendre le remet
            // à null plutôt que d'effacer l'historique du champ.
            since: paused ? nowIso : null,
            updatedAt: nowIso,
            // La raison est facultative et libre : c'est la parole de
            // l'apprenant, pas une catégorie que le produit lui impose.
            raison: typeof cmd.raison === 'string' ? cmd.raison.slice(0, 300) : '',
            provenance: {
              producer: validId(provenance?.producer, 60) ?? 'learner',
              method: typeof provenance?.method === 'string' ? provenance.method.slice(0, 80) : 'choix explicite',
            },
            schemaVersion: 2,
          },
        },
        effects: [`curriculum-pause:${paused ? 'on' : 'off'}`],
      };
    }
    case 'SET_SKILL': {
    ...
    case 'SET_MONTHLY_REVIEW': {
      ...
      // ── V75 · CP2 — D6 payée, ADDITIVEMENT ──
      //
      // Le CP0 a mesuré : forme imposée par ce handler, mais AUCUN horodatage,
      // AUCUNE provenance, AUCUNE version — et zéro lecteur dans l'interface.
      // Sans date, une revue hebdomadaire ne peut pas dire QUAND elle a été
      // faite, donc ne peut servir ni à une reprise ni à une télémétrie.
      //
      // `score` reste une AUTO-ÉVALUATION et n'est converti en rien d'autre :
      // le CP0 de V74 avait établi que ce produit souffre d'un excès de
      // déclaratif, et la réponse n'est pas de maquiller une déclaration en
      // mesure mais de la nommer. Le champ normalisé s'appelle `scoreDeclare` ;
      // `score` reste écrit pour que rien d'ancien ne casse.
      const rawScore = patch.score === null ? null
        : Number.isFinite(Number(patch.score)) ? Math.max(0, Math.min(5, Number(patch.score))) : base.score;
      const merged = normalizeWeeklyReview({
        done: typeof patch.done === 'boolean' ? patch.done : base.done,
        note: typeof patch.note === 'string' ? patch.note.slice(0, 4000) : base.note,
        score: rawScore,
        at: nowIso,
        provenance: isObj(cmd.provenance) ? cmd.provenance : { producer: 'learner', method: 'declaration' },
      }, { now: nowIso, legacy: false });
      // Compatibilité : l'ancien champ `score` reste présent et identique.
      merged.score = merged.scoreDeclare;
      return {
        ok: true,
...
export function applyCommand(progress, command, { now } = {}) {
  ...
  const base = {
    ...
    exerciseAttempts: normalizeExerciseAttempts(progress.exerciseAttempts),
  ...
  switch (type) {
    ...
    case 'RECORD_EXERCISE_ATTEMPT':
      return cmdRecordExerciseAttempt(base, command, nowIso);
    case 'RECORD_TRANSFER_ATTEMPT':
      return cmdRecordTransferAttempt(base, command, nowIso);
    case 'RECORD_HINT_VIEW':
      return cmdRecordHintView(base, command, nowIso);
    case 'RECORD_USAGE_EVENT':
      return cmdRecordUsageEvent(base, command, nowIso);
    case 'RECORD_ASSESSMENT_ATTEMPT':
      return cmdRecordAssessmentAttempt(base, command, nowIso);
    case 'RECORD_MISSION_SUBMISSION':
      return cmdRecordMissionSubmission(base, command, nowIso);
    case 'RECORD_ARTIFACT_ANALYSIS':
      return cmdRecordArtifactAnalysis(base, command, nowIso);
    default:
