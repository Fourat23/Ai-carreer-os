 ...
 */
/**
 * ── ORDRE DE SERVICE (V74 · CP8) ─────────────────────────────────────────
 *
 * À NE PAS CONFONDRE avec l'ordre d'ÉVALUATION gelé au §2 du contrat
 * (`UNKNOWN → OVERDUE → DUE → SOON → HEALTHY`), qui dit comment on CALCULE le
 * statut d'une fiche. Celui-ci dit dans quel ordre on SERT les fiches une fois
 * leur statut connu. Les deux sont indépendants, et le second ne touche pas au
 * premier.
 *
 * `UNKNOWN` passe APRÈS `DUE` mais AVANT `SOON` : une notion exposée et jamais
 * mise à l'épreuve est un trou réel — rien n'y a jamais échoué, donc rien n'y
 * alerte — alors qu'une notion `SOON` n'est simplement pas encore due.
 */
const RANG_STATUT = { OVERDUE: 0, DUE: 1, UNKNOWN: 2, SOON: 3, HEALTHY: 4 };

/**
 * ── CE QUE LE CP8 A CHANGÉ, ET POURQUOI ──────────────────────────────────
 *
 * Jusqu'au CP7 ce tri était `score`, puis statut, puis identifiant : le statut
 * n'était qu'un départage, et un score élevé suffisait à faire passer une
 * fiche `HEALTHY` devant une fiche `OVERDUE`.
 *
 * **La simulation du CP8 a mesuré ce que cela coûte.** Chez un apprenant qui
 * réussit 9 fois sur 10, **44 % des propositions portaient sur des notions
 * `HEALTHY`** — que le moteur lui-même déclare à jour — pendant qu'un arriéré
 * allant jusqu'à 28 notions en retard attendait. **222 places** avaient été
 * prises par une notion non due alors qu'une notion due attendait.
 *
 * La correction n'est pas un réglage, c'est une règle :
 *
 *   > **Une notion qui n'est pas due ne prend pas la place d'une notion qui
 *   > l'est.** Le statut décide de la BANDE, le score décide du rang DANS la
 *   > bande.
 *
 * Sa justification tient en une phrase : `INTERVALS` est l'échelle
 * d'espacement PUBLIÉE du produit, et le §4 du contrat gelé en confie la
 * responsabilité à V66. Laisser un score la contredire rendrait cette échelle
 * décorative — on afficherait « à revoir dans 16 jours » tout en proposant la
 * notion le lendemain.
 *
 * **Rien n'est exclu** : quand la file des notions dues est vide, les `SOON`
 * puis les `HEALTHY` remplissent le budget restant. Une session courte vaut
 * mieux qu'une session vide — mais elle ne se remplit qu'APRÈS le nécessaire.
 *
 * Le tri reste TOTAL — bande, score, identifiant — donc toujours déterministe
 * (critère B2).
 */
