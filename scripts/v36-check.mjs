#!/usr/bin/env node
// Gate V36 — lance : npm run v36:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V36 (cf.
// docs/architecture/v36-lessons-plan.json) : socle Frontend « Web Platform » (HTML
// sémantique, CSS cascade/box model, layout Flexbox/Grid, responsive, formulaires,
// TypeScript côté front) + leçons React durcies le cas échéant.
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V36.md).
//
...
const planPath = R('docs/architecture/v36-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V36 (socle Frontend Web Platform + React durci)');
  console.log('⚠️  plan v36-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V36 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V36 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
