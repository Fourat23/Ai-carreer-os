...
const FRONTEND_MODULE_SPECS = [
  ['fe-01-js', 'JavaScript moderne', 'Tableaux, fonctions, objets, asynchrone — les fondamentaux du langage de l\'interface.', 8, 14],
  ['fe-02-ts', 'TypeScript & conception', 'Typage, types avancés, POO, patterns, clean code, débogage — fiabiliser le code d\'interface.', 36, 49],
  ['fe-03-react-intro', 'React & consommation d\'API', 'Introduction à React, le front consomme l\'API (fetch et états async), une mini-app de bout en bout.', 87, 91],
  ['fe-04-react-core', 'React : composants, état et hooks', 'Composants/props/JSX, state (useState), rendu conditionnel et listes, effets (useEffect), formulaires contrôlés, gestion d\'erreur centralisée.', 92, 98],
  ['fe-05-react-app', 'Application React : routing, état partagé, performance, accessibilité', 'Routing et navigation, remontée et partage d\'état, Context, performance (re-renders), accessibilité et UX.', 99, 105],
  ['fe-06-tests', 'Tests & qualité front', 'Tests unitaires, tests de composants React, mocks et intégration, clean code appliqué au front, hooks personnalisés, gestion d\'erreur robuste.', 106, 112],
  ['fe-07-project', 'Projet 3 — BiblioApp (intégrateur)', 'Construire une application front complète : socle, CRUD, recherche et filtres, tests, polish et états limites, README/ADR/démo.', 113, 119],
];
const FRONTEND_TECHS = ['javascript', 'typescript', 'react', 'testing'];

/** Modules du parcours Frontend : plages de jours réelles, skills dérivés. */
function frontendModules(program) {
  const days = program?.days ?? [];
  return FRONTEND_MODULE_SPECS.map(([id, title, summary, from, to]) => {
    const mdays = days.filter((d) => d.day >= from && d.day <= to).sort((a, b) => a.day - b.day);
    return { id, title, summary, dayRefs: mdays.map((d) => d.day), skills: uniq(mdays.map((d) => d.skill).filter(Boolean)), projectRef: null };
  });
}
