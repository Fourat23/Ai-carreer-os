// CP7 (V29) — contrat des nouveaux exercices Data/SE/Git, vérifié par EXÉCUTION :
...
const V29_EXERCISES = [
  'sql-inner-join', 'db-concurrency-risk', 'migration-compat',
  'breaking-change-classify', 'git-commit-grouping',
];

const tmp = mkdtempSync(join(tmpdir(), 'v29x-'));
let seq = 0;
...
test('V29 : les 5 exercices existent et sont des fixtures valides', () => {
  for (const id of V29_EXERCISES) {
...
test('V29 : compétences connues, ≥1 test public et ≥1 test privé, référence présente', () => {
  for (const id of V29_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V29 : référence 100% verte, starter échoue ≥1 test public (exécuté)', async () => {
  for (const id of V29_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
