...
/** Dimensions qu'une grille d'évaluation peut distinguer (optionnel par critère). */
export const RUBRIC_CATEGORIES = ['functional', 'quality', 'maintainability', 'tests', 'performance', 'documentation', 'security', 'tradeoffs'];
...
export function validateMission(m = {}, ctx = {}) {
  ...
  if (m.rubric != null) {
    if (!isArr(m.rubric) || m.rubric.length > MAX.rubric) errors.push('rubric hors bornes');
      ...
      if (r?.category != null && !RUBRIC_CATEGORIES.includes(r.category)) errors.push(`rubric : catégorie inconnue « ${r.category} »`);
    }
  }
...
export function publicMissionView(m) {
  return {
    ...
    deliverables: (m.deliverables ?? []).map((d) => ({
      ...
      // Lien PUBLIC vers l'exercice de lab (jamais la solution ni les tests privés).
      exerciseId: d.exerciseRef ?? undefined,
      // Sections attendues = critères d'acceptation publics (pas un attendu caché).
      // Les seuils cachés (requireMentions, minLength) ne sont PAS exposés.
      requiredSections: d.docSpec?.requiredSections ?? undefined,
    })),
    rubric: (m.rubric ?? []).map((r) => ({ label: r.label, blocking: !!r.blocking, category: r.category ?? undefined })),
  };
}
