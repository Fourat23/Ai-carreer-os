...
- **Objectif structurel atteint : 313/313 exemples guidés.** Tous les jours de travail de 1 à 365 disposent désormais d'un exemple guidé exécutable, d'une question d'entretien réelle, d'un cas métier, d'une section bankable et d'une correction exigeante. La tranche 61-90 était la dernière sans enrichissement.
- **Tranche fidèle au programme réel** : les titres, objectifs, exercices et livrables réels des jours 61-90 (`days-31-90.mjs`) n'ont pas été modifiés ; l'enrichissement complète la théorie existante (`days-31-90-extras.mjs`, rendue en `theoryExtra`) sans l'écraser.
- **Même gabarit complet** que 91-365 : cours + **modèle mental distinct** par jour, exemple guidé exécutable (avec exemple simple + raisonnement pas-à-pas), **mini-quiz** (4 questions, réponses en correction), cas métier distinct, question d'entretien avec réponse attendue + relance, section bankable, correction exigeante (logique + solution simple + améliorée, pièges, vérifications, oral), liens vers les leçons de fond ciblées (express-backend, sql-foundations, database-modeling, testing-foundations, authentication, deployment-secrets, error-handling, http-rest-json, system-design-interview, terminal-shell-filesystem, git-advanced, readme-documentation, technical-storytelling, clean-code, architecture-basics, api-design-basics, design-patterns-intro, observability-logging, monitoring-production, caching-performance, python-foundations, portfolio-github, interview-preparation, react-fundamentals, react-hooks-effects, async-javascript).
- **Progression pédagogique** : chaque jour s'appuie sur les précédents (le socle CRUD du 61 porte les emprunts transactionnels du 62 ; la sécurité du 67 et l'auth du 68 durcissent LivreAPI au 85 ; l'immutabilité du 87 conditionne le fetch du 88 puis le full-stack du 89 ; les trade-offs du 81 et le storytelling du 86 préparent l'entretien ; le bilan du 90 relie explicitement la data/ML/IA à venir aux fondations acquises — « le RAG = ingestion + API + évaluation »). Fil rouge : « au-delà de faire fonctionner le code, structurer, sécuriser, rendre observable, arbitrer par trade-offs, et devenir un développeur full-stack junior employable qui construit sur ses fondations vers l'IA ».
- **Audit anti-générique (passe Opus 4.8)** : les **26 modèles mentaux** vérifiés tous distincts et propres au titre de chaque jour ; **0 question d'entretien dupliquée**, **0 cas métier dupliqué** (vérifié programmatiquement). Les jours proches différenciés : 66 (doc d'API/démo, projet) vs 74 (documentation technique générale, docstrings-contrat/diagrammes) ; 78 (3-tiers/MVC) vs 76 (modularité couplage/cohésion) ; 83 (consolidation mois 3, familiarité≠maîtrise) vs 90 (clôture trimestre, employabilité + cap data/IA) ; 88 (fetch/3 états async) vs 89 (full-stack assemblé, trajet complet).
- **Mécanisme** : nouveau `scripts/data/days-enrich-61-90.mjs` (ENRICH_61_90), fusionné dans `generate-curriculum.mjs` (import + ajout au merge DAYS_ENRICH). La branche 31-90 du générateur consommait déjà tous les champs enrich (câblés au Batch 5G). Overrides `lessons` par jour.
- Checks : `curriculum:check` ✅ · `curriculum:depth-check` ✅ (exemples guidés **313/313**, vs 287 avant ce batch) · tests **43/43** ✅ · build ✅.
- **Périmètre strictement respecté** : aucun jour hors 61-90 modifié ; jours 1-60 et 91-365 inchangés ; interface, glossaire, leçons et architecture générale inchangés (seuls le nouveau fichier d'enrichissement et deux lignes d'import/merge du générateur ont été touchés).

## 0-duodecies. Mise à jour — mois 2 & 3 enrichis, jours 31-60 (Batch 5G terminé)

- **26 jours d'apprentissage enrichis en profondeur** (31-34, 36-41, 43-48, 50-55, 57-60) + **4 revues enrichies** (35, 42, 49, 56). Sous-batchs : 5G1 (31-40), 5G2 (41-50), 5G3 (51-60). Couvre les **structures de données appliquées** (réflexe hash map/twoSum, backtracking, stacks/queues, listes chaînées/BST), le **TypeScript et la POO** (typage-contrat, types avancés/états illégaux irreprésentables, POO/polymorphisme, design patterns, clean code, débogage méthodique), la **programmation fonctionnelle typée** et le **Projet 1 TaskFlow** de la conception à la finition (interface Store/inversion de dépendance, JsonStore robuste + ids max+1, CRUD immuable + filtres, tests/README/ADR), la **consolidation/entretien blanc**, puis le mois 3 **HTTP/REST/Express** (HTTP sans état, REST design prévisible, Node natif→Express, middlewares + 3 couches, validation + erreurs centralisées), le **SQL** (déclaratif/JOIN/GROUP BY/WHERE vs HAVING), **SQLite branché** (requêtes paramétrées anti-injection, couche data isolée), la **modélisation** (normalisation/index/transactions ACID), **Postman avancé** et le **cadrage du Projet 2 LivreAPI** (table de liaison, contrat avant le code).
- **Tranche fidèle au programme réel** : les titres, objectifs, exercices et livrables réels des jours 31-60 (`days-31-90.mjs`) n'ont pas été modifiés ; l'enrichissement complète la théorie existante (`days-31-90-extras.mjs`, rendue en `theoryExtra`) sans écraser aucun contenu.
- **Même gabarit complet** que 91-365 : cours théorique + **modèle mental distinct** par jour, exemple guidé exécutable (avec exemple simple + raisonnement pas-à-pas), **mini-quiz** (4 questions avec réponses en correction), cas métier distinct, question d'entretien avec réponse attendue + relance, section bankable, correction exigeante (logique + solution simple + améliorée, pièges, vérifications, section orale), liens vers les leçons de fond ciblées (data-structures-intro, algorithmic-thinking, recursion, typescript-basics, design-patterns-intro, clean-code, testing-foundations, architecture-basics, error-handling, http-rest-json, api-design-basics, express-backend, sql-foundations, database-modeling, readme-documentation, interview-preparation).
- **Progression pédagogique** : chaque jour s'appuie sur les précédents (le réflexe hash map du 31 et le backtracking du 32 arment l'entretien blanc du 48 ; l'interface Store du 44 est implémentée au 45 puis migrée sur SQLite au 57 — preuve concrète de l'inversion de dépendance ; le paramétrage du 55 devient la défense anti-injection du 57 ; le REST du 51 et la modélisation du 58 cadrent le Projet 2 au 60). Fil rouge mois 2 : « du code qui marche à du code sûr, lisible, testable et diagnosticable » ; fil rouge mois 3 : « construire une API = comprendre HTTP, concevoir un contrat REST cohérent, structurer en couches, se défendre (validation/erreurs), interroger ses données (SQL) ».
- **Audit anti-générique (passe Opus 4.8)** : les **26 modèles mentaux** vérifiés tous distincts et propres au titre de chaque jour ; **0 question d'entretien dupliquée**, **0 cas métier dupliqué** (vérifié programmatiquement sur 51-60) ; exemples guidés réellement liés au titre (twoSum, parenthèses/pile, BST inorder, injection `' OR 1=1 --` neutralisée, transaction stock+emprunt, etc.). Les jours au thème proche différenciés : 44 (conception types + Store) vs 45 (implémentation JsonStore) vs 47 (finition tests/README/ADR) ; 52 (Node natif vs Express) vs 53 (Express structuré en couches) ; 55 (SQL requêtes) vs 57 (SQLite branché + sécurité) vs 58 (modélisation/index/transactions).
- **Mécanisme** : `scripts/data/days-enrich-31-60.mjs` (ENRICH_31_60), fusionné dans `generate-curriculum.mjs` (import + ajout au merge DAYS_ENRICH). La branche 31-90 du générateur consomme désormais `enrich.future`, `enrich.solution` et `enrich.quiz` (câblage minimal, sans effet sur les jours 61-90 dont l'enrichissement reste vide). Overrides `lessons` par jour.
- Checks : `curriculum:check` ✅ · `curriculum:depth-check` ✅ (exemples guidés **287/313**, vs 261 avant ce batch) · tests **43/43** ✅ · build ✅.
- **Périmètre strictement respecté** : aucun jour hors 31-60 modifié ; jours 1-30 et 61-365 inchangés ; interface, glossaire, leçons et architecture générale inchangés (seuls le nouveau fichier d'enrichissement et le câblage du générateur ont été touchés).

## 0-undecies. Mise à jour — machine learning classique & Projet 5 ChurnScope jours 151-180 (Batch 5F terminé)

- **26 jours d'apprentissage enrichis en profondeur** (151-153, 155-160, 162-167, 169-174, 176-180) + **4 revues enrichies** (154, 161, 168, 175). Sous-batchs : 5F1 (151-160), 5F2 (161-170), 5F3 (171-180). Couvre : les **statistiques/probabilités pour le ML** (Bayes et piège du taux de base, biais d'échantillonnage — sélection/survivant/historique/déséquilibre, étude statistique honnête), le **workflow scikit-learn** (API fit/predict/score, modèles interchangeables), la **régression** (linéaire et interprétation des coefficients, train/test + baseline + **démonstration du data leakage**, métriques MAE/RMSE/R², régression logistique — sigmoïde/seuil/matrice de confusion, rapport de modèle orienté décision), la **classification** (métriques précision/rappel/F1/AUC selon le coût métier, arbres de décision, random forests — bagging/vote, cross-validation — moyenne±variabilité, overfitting et compromis biais-variance/régularisation, analyse d'erreurs qualitative), la **préparation et l'exploitation** (feature engineering — hypothèses métier mesurées, encodage one-hot vs label et préprocessing anti-leakage, pipelines scikit-learn — anti-leakage structurel et déployable, clustering k-means — normaliser/choisir k/interpréter, interprétabilité — permutation importance/détection de leakage, cadrage ML depuis la décision métier), et le **Projet 5 ChurnScope** de bout en bout (EDA + baseline honnête, comparaison de modèles avec journal d'expériences, feature engineering avec anti-leakage temporel, réglage d'hyperparamètres avec test final intouché, rapport orienté décision + analyse d'erreurs qualitative).
- **Tranche authentiquement ML** : les titres réels des jours 151-180 (`days-plan.mjs`, semaines 22-26) correspondent bien au thème machine learning des mois 6-7. Les exigences pédagogiques (concepts expliqués depuis les fondations, exemples exécutables scikit-learn, rattachement à la décision métier) ont été appliquées ; les jours de projet (176-180) rattachent chaque journée à une étape réelle avec livrable vérifiable et rigueur anti-leakage.
- **Même gabarit complet** que 181-365 : cours théorique + **modèle mental distinct** par jour, exemple guidé exécutable (avec exemple simple + mini-quiz), cas métier distinct, question d'entretien avec réponse attendue, section bankable, correction exigeante (solution simple + améliorée, pièges, vérifications, section orale), liens vers les leçons de fond ciblées (statistics-for-ml, machine-learning-basics, scikit-learn-workflow, model-evaluation, feature-engineering, data-cleaning-quality).
- **Progression pédagogique** : chaque jour s'appuie sur les précédents (Bayes/taux de base du 151 fonde la lecture des métriques déséquilibrées du 162 ; le leakage du 157 se retrouve dans le préprocessing du 170, les pipelines du 171 et le feature engineering du 178 ; la baseline du 157 et le cadrage du 174 structurent tout ChurnScope ; l'analyse d'erreurs du 167 nourrit le rapport final du 180). Fil rouge : « le ML est un PROCESSUS rigoureux — évaluation honnête sans leakage, métrique selon le coût métier, communication orientée décision — où l'algorithme est un composant interchangeable ».
- **Audit anti-générique (passe Opus 4.8)** : les **26 modèles mentaux** vérifiés tous distincts et propres au titre de chaque jour ; **aucune question d'entretien dupliquée** ; cas métier propres à chaque sujet ; tous les jours ≥ 1202 mots. Les jours au thème proche ont été différenciés : 160 (rapport d'UN modèle) vs 180 (conclusion de PROJET avec analyse d'erreurs qualitative) ; 169 (feature engineering conceptuel) vs 178 (feature engineering appliqué avec anti-leakage temporel) ; 157 (leakage général) vs 171 (Pipeline anti-leakage structurel).
- **Mécanisme** : `scripts/data/days-enrich-151-180.mjs` (ENRICH_151_180), fusionné dans `generate-curriculum.mjs` (import + ajout au merge DAYS_ENRICH). Overrides `lessons` par jour.
- Checks : `curriculum:check` ✅ · `curriculum:depth-check` ✅ (exemples guidés **261/313**, vs 235 avant ce batch) · tests **43/43** ✅ · build ✅.
- **Périmètre strictement respecté** : aucun jour hors 151-180 modifié ; interface, glossaire, leçons et architecture générale inchangés (seuls le nouveau fichier d'enrichissement et deux lignes d'import/merge du générateur ont été touchés).
...
- **Tranche authentiquement Python/data** : contrairement aux jours 91-120 (dont la consigne « Python/data » était factuellement fausse), les titres réels des jours 121-150 (\`days-plan.mjs\`, semaines 18-22) correspondent bien au thème data. Les exigences pédagogiques data (concepts expliqués depuis zéro, comparaisons JS/SQL/pandas où elles aident, références/mutations/structures précises, exemples exécutables, rattachement progressif aux usages ML) ont été appliquées ; les jours de projet (141-146) rattachent chaque journée à une fonctionnalité réelle avec livrable vérifiable, tests, documentation et critères de qualité.
- **Même gabarit complet** que 181-365 : cours théorique + **modèle mental distinct** par jour, exemple guidé (exemple simple + mini-quiz), cas métier distinct, question d'entretien avec réponse attendue, section bankable, correction exigeante (solution simple + améliorée, pièges, vérifications, section orale), liens vers les leçons de fond ciblées (python-foundations, error-handling, clean-code, testing-foundations, pandas-data-wrangling, data-cleaning-quality, database-modeling, sql-foundations, etl-pipelines, statistics-for-ml, architecture-basics, readme-documentation).
- **Progression pédagogique** : chaque jour s'appuie sur les précédents (les fonctions pures du jour 132 servent le transform de DataPulse au 143 ; le schéma 3NF du 134, la transaction du 136 et l'idempotence du 139 se combinent au load du 144 ; les 3 questions du cadrage 141 dictent extract/transform/dashboard ; la dispersion du 148 prépare la visualisation du 149 puis la causalité du 150). Fil rouge : « du code data professionnel est organisé, robuste, testé et reproductible » et « une analyse vaut par ses questions, la traçabilité de son traitement et sa reproductibilité ».
- **Audit anti-générique (passe Opus 4.8)** : les **26 modèles mentaux** vérifiés tous distincts et propres au titre de chaque jour ; **aucune question d'entretien dupliquée** ; cas métier propres à chaque sujet ; tous les jours ≥ 1107 mots (niveau des meilleurs 181-365). Le jour 146 (README/ADR/démo de DataPulse) réutilise le principe « un projet non documenté n'existe pas pour un recruteur » mais avec un contenu entièrement distinct des jours 118/272 (projet de pipeline data, ADR SQLite-vs-PostgreSQL, README orienté questions/réponses/reproductibilité, démo « rejouable en une commande »). **Correction post-audit** : la revue 133 (semaine 19 pandas), initialement omise du sous-batch 5E2, a été ajoutée et vérifiée (synthèse + grille + remédiation + entretien).
- **Mécanisme** : \`scripts/data/days-enrich-121-150.mjs\` (ENRICH_121_150), fusionné dans \`generate-curriculum.mjs\` (import + ajout au merge DAYS_ENRICH). Overrides \`lessons\` par jour pour pointer les leçons Python/data/SQL/stats pertinentes.
- Checks : \`curriculum:check\` ✅ · \`curriculum:depth-check\` ✅ (exemples guidés **235/313**, vs 209 avant ce batch) · tests 20/20 ✅ · build ✅.
- **Périmètre strictement respecté** : aucun jour hors 121-150 modifié ; interface, architecture générale et leçons inchangées (seuls le nouveau fichier d'enrichissement et deux lignes d'import/merge du générateur ont été touchés).
