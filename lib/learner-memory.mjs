...
import { normalizeTransferAttempts } from './transfer-attempt.mjs';
 ...
 */
export function collectContacts({ days = {}, recallAttempts = [], exerciseAttempts = [], evidence = [], transferAttempts = [] } = {}) {
  const contacts = [];
  ...
  // ── M5 · V75 · CP10 — LA TENTATIVE DE TRANSFERT, RÉUSSIE OU NON ────────
  //
  // Jusqu'au CP9 inclus, le seul signal de transfert était une PREUVE — donc
  // un succès, et seulement s'il avait été conservé. Un défi raté ne laissait
  // aucune trace, alors que **c'est l'échec qui porte l'information** : il dit
  // que la notion tient dans son contexte d'origine et cède ailleurs.
  //
  // Le profil R du CP0 (« recall OK, transfert KO ») était littéralement
  // inobservable avant ce contact.
  for (const a of normalizeTransferAttempts(transferAttempts)) {
    contacts.push({
      kind: 'transfer', at: a.at, conceptId: null,
      competencyIds: a.competencyIds ?? [],
      conceptIds: a.conceptIds ?? [],
      outcome: a.outcome === 'success' ? 'recalled' : a.outcome === 'partial' ? 'partial' : 'failed',
      challengeId: a.challengeId, dayRef: null, ref: a.challengeId,
...
function ficheDe(id, contacts, expositions, ctx) {
  ...
  // ── V75 · CP10 — LE COMPTEUR LIT MAINTENANT LE FAIT, PAS SEULEMENT SA
  //    PROJECTION ──
  //
  // Deux sources disent « ce défi a été réussi » : la TENTATIVE (fait écrit
  // systématiquement depuis le CP10) et la PREUVE (projection, écrite seulement
  // si l'apprenant conserve son résultat). On les UNIONNE par `challengeId`
  // plutôt que de les additionner — sans quoi un succès conservé compterait
  // deux fois.
  //
  // Pourquoi ne pas se contenter du fait, maintenant qu'il existe : parce
  // qu'une progression écrite entre le CP9 et le CP10 contient des preuves de
  // transfert **sans** tentative correspondante. Les ignorer effacerait une
  // histoire réelle, ce que le contrat interdit.
  const defisReussis = new Set();
  for (const c of contacts) {
    if (c.kind === 'transfer' && c.outcome === 'recalled') defisReussis.add(c.challengeId);
    if (c.kind === 'evidence' && c.sourceType === 'transfer-challenge') defisReussis.add(c.ref ?? '?');
  }
  const transferts = [...defisReussis];
  // Les défis TENTÉS sans succès. C'est le signal que le CP0 ne pouvait pas
  // voir : « la notion tient chez elle et cède ailleurs » (profil R).
  const transfertsEchoues = contacts.filter((c) => c.kind === 'transfer' && c.outcome !== 'recalled');
  ...
  return {
    ...
    // Défis de transfert RÉUSSIS, comptés une fois chacun. La dette D10 étant
    // levée (CP9) et le fait persisté (CP10), ce compteur peut enfin être
    // non nul — et il dit « ce défi-ci a été réussi », pas « la compétence est
    // maîtrisée ».
    transfers: transferts.length,
    // V75 · CP10 — tentatives de transfert NON réussies. L'échec au transfert
    // était jusqu'ici invisible ; c'est pourtant lui qui distingue « su » de
    // « transférable ».
    transfersEchoues: transfertsEchoues.length,
