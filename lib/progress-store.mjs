...
import { normalizeArtifactAnalyses, MAX_ARTIFACT_ANALYSES } from './artifact-analysis.mjs';
...
export function normaliserLesFaits(src, { daysPourHeritage = null } = {}) {
  return {
    ...
    // V77 · CP7 · ARTEFACTS ANALYSÉS. Quatre surfaces validaient et analysaient
    // réellement un artefact RÉDIGÉ par l'apprenant, et n'écrivaient pas un
    // octet. Le fait dit combien de diagnostics ont été rendus ; il ne dit
    // jamais que l'artefact est juste — un compte n'est pas un verdict.
    artifactAnalyses: normalizeArtifactAnalyses(src?.artifactAnalyses).slice(-MAX_ARTIFACT_ANALYSES),
