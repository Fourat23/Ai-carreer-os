 ...
 */
/**
 * ── LA RÉVISION D'UN CONTENU ─────────────────────────────────────────────
 *
 * Empreinte courte et STABLE : même contenu ⇒ même révision, sur n'importe
 * quelle machine et à n'importe quel moment. Un horodatage ne le garantirait
 * pas, et une sauvegarde identique changerait de révision pour rien.
 */
...
export function readWorkspaceTree(root, exercise) {
  ...
  for (const [path, meta] of templateFileMap(exercise)) {
    ...
    // ── V76 · CP9 — LA RÉVISION D'UN FICHIER ──
    //
    // Empreinte du CONTENU, pas horodatage : deux serveurs, deux horloges, le
    // même contenu doivent donner la même révision. Elle sert à refuser une
    // sauvegarde dont la base est périmée — le CP9 a mesuré qu'un onglet laissé
    // ouvert une heure écrasait silencieusement le travail d'un autre.
    out.push({
      path, content, readOnly: !meta.editable, editable: meta.editable,
      hidden: meta.hidden, language: meta.language, entry: meta.entry,
      rev: revisionDe(content),
    });
  }
  return out;
}
