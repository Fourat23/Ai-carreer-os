...
export function remedier({
  ...
  diagnostic = null,
  correctionVue = false,
  // V76 · CP7 — les marches DÉJÀ LUES sur cet exercice. Une échelle qui
  // repropose à l'identique ce que l'apprenant vient de lire ne monte pas :
  // elle piétine. Le CP0 avait mesuré que rien n'enregistrait ces lectures.
  dejaVues = [],
} = {}) {
  ...
  // ── V76 · CP7 — NE PAS REPROPOSER CE QUI A DÉJÀ ÉTÉ LU ──
  //
  // On ÉCARTE les marches déjà consultées, mais jamais au point de ne plus rien
  // avoir à offrir : si toutes l'ont été, on garde la liste complète et
  // l'échelle continue par son niveau. Vider les marches ferait sauter à la
  // correction, c'est-à-dire exactement l'inverse de ce que le contrat exige.
  const vues = new Set(Array.isArray(dejaVues) ? dejaVues : []);
  const neuves = dispo.filter(Boolean).filter((m) => !vues.has(m.action));
  const offre = neuves.length ? neuves : dispo.filter(Boolean);
  const parAction = new Map(offre.map((m) => [m.action, m]));
