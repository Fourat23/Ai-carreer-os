#!/usr/bin/env node
// Gate V37 — lance : npm run v37:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V37 (cf.
// docs/architecture/v37-lessons-plan.json) : socle Frontend « Web Platform » (HTML
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V37.md).
//
...
const planPath = R('docs/architecture/v37-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V37 (profondeur Frontend + Next.js)');
  console.log('⚠️  plan v37-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V37 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V37 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
