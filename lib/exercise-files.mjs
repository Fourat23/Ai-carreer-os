...
export function clientFiles(files) {
  return (Array.isArray(files) ? files : [])
    .filter((f) => !f.test)
    // `rev` voyage jusqu'au client : c'est LUI qui la renverra pour prouver sur
    // quelle base il a travaillé (V76 · CP9).
    .map((f) => ({ path: f.path, content: f.content, language: f.language, editable: f.editable, hidden: f.hidden, entry: f.entry, rev: f.rev ?? null }));
}
