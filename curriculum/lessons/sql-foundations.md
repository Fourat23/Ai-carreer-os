...
Un **index** est un arbre auxiliaire qui rend une recherche O(log n) au lieu du parcours complet — au prix d'espace et d'écritures plus lentes : on indexe les colonnes FILTRÉES/JOINTES souvent, pas tout. Une **transaction** rend un groupe d'opérations « tout ou rien » : sans elle, une panne entre « créer la commande » et « décrémenter le stock » laisse la base incohérente. On résume ses quatre garanties par le sigle **ACID**, et chaque lettre achète quelque chose de précis : **A**tomicité (le groupe passe entier ou pas du tout), **C**ohérence (les contraintes de la base restent vraies après comme avant), **I**solation (deux transactions simultanées ne se voient pas à moitié faites), **D**urabilité (une fois confirmée, la donnée survit à une coupure de courant). Retiens surtout l'atomicité et l'isolation : ce sont les deux qui expliquent la quasi-totalité des incidents de données.
