// CP8 (V36) — contrat des exercices frontend ajoutés, vérifié par EXÉCUTION via
...
test('V36 : compétences connues, ≥1 test public et ≥1 test privé, référence présente', () => {
  for (const id of V36_EXERCISES) {
    const ex = JSON.parse(read(`data/exercises/${id}.json`));
...
test('V36 : exercices reliés à un jour réel (reachables)', () => {
  const dayEx = JSON.parse(read('data/day-exercises.json'));
  const reachable = new Set(Object.values(dayEx).flat());
  for (const id of V36_EXERCISES) assert.ok(reachable.has(id), `${id} : relié à un jour`);
});
