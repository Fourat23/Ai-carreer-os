// CP2 (V31) — intégrité du dispositif d'audit pédagogique V31 : plan, ledger,
...
const plan = JSON.parse(readFileSync(R('docs/architecture/v31-lessons-plan.json'), 'utf8'));
const ledger = JSON.parse(readFileSync(R('docs/architecture/v31-pedagogy-audit.json'), 'utf8'));
...
test('plan V31 : structure et slugs valides', () => {
  assert.ok(Array.isArray(plan.newLessons), 'newLessons est un tableau');
  assert.ok(Array.isArray(plan.hardenedLegacy), 'hardenedLegacy est un tableau');
  assert.ok(Array.isArray(plan.critical), 'critical est un tableau');
  for (const slug of plan.newLessons) assert.ok(bySlug.has(slug), `nouvelle leçon existe : ${slug}`);
  for (const slug of plan.hardenedLegacy) assert.ok(bySlug.has(slug), `historique existe : ${slug}`);
  for (const slug of plan.critical) assert.ok([...plan.newLessons, ...plan.hardenedLegacy].includes(slug), `critique dans le périmètre : ${slug}`);
});
...
test('ledger V31 : valide contre la rubrique partagée', () => {
