...
export function submitDeliverable(flat, missionDef, deliverableId, patch = {}, now = new Date().toISOString()) {
  ...
  // Historique minimal des soumissions (borné) pour la traçabilité.
  const prevHistory = Array.isArray(prev.history) ? prev.history : [];
  entry.history = [...prevHistory, { status: patch.status, at: now }].slice(-20);
...
export function recordMissionCompletion(flat, missionDef, now = new Date().toISOString()) {
  ...
  //
  // V65 avait déjà retiré l'écriture directe d'un niveau de compétence (P2) :
  // la mission produisait une PREUVE, et la compétence s'en projetait. Restait
  // le mot lui-même. Le CP0 a mesuré ce qu'il recouvrait :
  //
  //   · **42 missions sur 42** terminent sur une revue que l'apprenant signe
  //     lui-même, d'un clic ;
  //   · un document décrivant une conception délibérément mauvaise obtient
  //     `structure ok: true`.
  //
  // Une preuve `passed` plaçait donc ce clic au même rang qu'un exercice dont
  // les tests ont tourné en bac à sable. La règle du maillon faible (contrat
  // CP1 §2) l'interdit : `STRUCTURE_VALID + SELF_CONFIRMATION` vaut sa
  // composante la plus faible.
  //
  // `manual` est le statut du vocabulaire existant qui dit exactement cela :
  // *constaté, par un moyen qui n'est pas une correction automatique.* La
  // preuve reste écrite, datée, exportable et visible — elle cesse seulement
  // de prétendre démontrer une compétence.
  //
  // **Ce n'est pas une migration.** Les preuves déjà sur le disque gardent leur
  // `status` d'origine ; seules les missions terminées à partir d'ici sont
  // décrites autrement.
  const ev = makeEvidence({
    sourceType: 'mission',
    sourceId: missionDef.id,
    competencyIds: missionDef.skills ?? [],
    validation: {
      status: 'manual',
      kind: 'mission-deliverables',
      checkedAt: now,
      detail: 'Livrables requis complétés — structure vérifiée et revue signée par l’apprenant.',
    },
    title: `Mission terminée : ${missionDef.title}`,
    provenance: {
      producer: 'mission-engine',
      method: 'mission-deliverables',
      note: 'Auto-vérifiés, structure validée, revue AUTO-DÉCLARÉE : travail documenté, non évalué.',
    },
    // ── V77 · CP10 — LA FILIATION, ÉCRITE PLUTÔT QUE REDÉCOUVERTE ──
    //
    // Les livrables de mode `auto` sont validés par la résolution d'un exercice
    // (`reconcileAutoDeliverables`). Le CP10 a mesuré 42 paires de ce type, dont
    // 14 chevauchent une compétence canonique. Cette mission a donc été terminée
    // EN PARTIE grâce à un travail déjà compté ailleurs, et la preuve le dit.
    //
    // Ce n'est pas une règle de déduplication : aujourd'hui il n'y a rien à
    // dédupliquer (0 double comptage effectif). C'est la donnée dont une telle
    // règle aurait besoin si elle devenait nécessaire — au lieu de la deviner.
    derivedFrom: (missionDef.deliverables ?? [])
      .filter((d) => d.validation === 'auto' && d.exerciseRef)
      .map((d) => `exercise:${d.exerciseRef}`),
    dayId: Number.isInteger((missionDef.dayRefs ?? [])[0]) ? missionDef.dayRefs[0] : undefined,
    artifactRef: url,
  }, { now });

  let ledger = flat.evidence ?? [];
  if (ev.ok) ledger = appendEvidence(ledger, ev.evidence).evidence;

  return { ...flat, days, evidence: ledger };
}
...
/**
 * Revue HONNÊTE (pure) d'une mission : ce qui est auto-validé, structurellement
 * valide, ou en attente de revue humaine. Le « score » n'est qu'un taux de
 * COMPLÉTION des livrables requis — jamais un jugement de qualité.
 */
export function missionReview(missionDef, state) {
  const s = { ...emptyMissionState(), ...state };
  const rankOf = (d) => RANK[s.deliverables?.[d.id]?.status ?? 'todo'];
  const autoValidated = [];
  const structureValid = [];
  const awaitingReview = [];
  const todo = [];
  for (const d of missionDef.deliverables ?? []) {
    const st = s.deliverables?.[d.id]?.status ?? 'todo';
    if (d.validation === 'auto' && st === 'validated') autoValidated.push(d.id);
    else if (d.validation === 'structural' && rankOf(d) >= RANK['structure-valid']) structureValid.push(d.id);
    else if (d.validation === 'review' && st !== 'validated') awaitingReview.push(d.id);
    else if (rankOf(d) === 0) todo.push(d.id);
  }
  const required = (missionDef.deliverables ?? []).filter((d) => d.required);
  const done = required.filter((d) => rankOf(d) >= doneTargetRank(d.validation)).length;
  return {
    status: computeMissionStatus(missionDef, s),
    // Taux de complétion des livrables requis (0..1) — PAS une note de qualité.
    completion: required.length ? done / required.length : 0,
    autoValidated, structureValid, awaitingReview, todo,
    // Critères de rubric bloquants : rappelés, jamais auto-jugés sur le fond.
    blockingCriteria: (missionDef.rubric ?? []).filter((r) => r.blocking).map((r) => r.label),
    humanReviewRequired: awaitingReview.length > 0,
  };
}
