// CP9 (V29) — E2E déterministe du socle + Frontend/React + Data/SQL + SE : les leçons
...
const FRONTEND = [
  'browser-dom-rendering', 'react-fundamentals', 'react-hooks-effects',
  'react-composition-architecture', 'react-accessibility',
];

test('E2E-1 : le parcours de connaissance Frontend/React existe (fichier + program.json)', () => {
  const bySlug = new Map(program.lessons.map((l) => [l.slug, l]));
  for (const slug of FRONTEND) {
    assert.ok(existsSync(R(`curriculum/lessons/${slug}.md`)), `fichier ${slug}`);
    assert.ok(bySlug.has(slug), `program.json contient ${slug}`);
  }
});

test('E2E-2 : chaque leçon critique V29 est reliée à la pratique', () => {
  const bySlug = new Map(program.lessons.map((l) => [l.slug, l]));
...
test('E2E-5 : les parcours Frontend/Data restent ANNONCÉS (pas de greenwashing)', () => {
  ...
  for (const id of ['frontend-engineer-v1', 'data-ml-v1']) {
    const t = byId.get(id);
