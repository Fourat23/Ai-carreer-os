#!/usr/bin/env node
// Gate V30 — lance : npm run v30:check
//
// Valide STRUCTURELLEMENT les leçons du périmètre V30 (cf.
// docs/architecture/v30-lessons-plan.json) : NOUVELLES leçons (documentation SE, etc.) ET
// leçons HISTORIQUES corrigées (Backend/API + dette AI/ML historique + SE).
...
// rapport CP11 (docs/PEDAGOGICAL-AUDIT-V30.md).
//
...
const planPath = R('docs/architecture/v30-lessons-plan.json');
if (!existsSync(planPath)) {
  console.log('── Gate V30 (Backend/API + AI/ML + documentation SE) ──');
  console.log('⚠️  plan v30-lessons-plan.json absent : rien à valider (attendu avant CP2).');
  console.log('\n✅ V30 valide (aucun périmètre déclaré).');
  process.exit(0);
}
...
console.log('\n✅ V30 valide : périmètre conforme (on-ramp, prérequis, vocabulaire, liens, pratique, graphe acyclique, réel/simulé).');
