...
// Un runtime décrit COMMENT exécuter du code : binaire autorisé, limites. La
// liste est fermée : aucun exercice ne peut introduire un binaire arbitraire.
export const RUNTIMES = {
  'node-js': {
    id: 'node-js',
    kind: 'node',
    label: 'JavaScript (Node.js)',
    binary: 'node',                  // seul binaire autorisé pour ce runtime
    fileExtension: '.mjs',
    timeoutMs: 5000,                 // temps mur maximal d'une exécution
    maxOutputBytes: 100_000,         // sortie tronquée au-delà
  },
};
...
export function validateExercise(ex) {
  ...
  if (!getRuntime(ex.runtime)) fail(`Exercice : runtime inconnu « ${ex.runtime} ».`);
...
export function checkTest(test, observed = {}, error = null) {
  const base = { testId: test.id, name: test.name, expected: test.expected };
